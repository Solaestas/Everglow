...
public class KillNPCObjectiveTest
{
	...
	public void IndividualProgress_Should_CalculateProperly()
	{
		for (int reqCount = 1; reqCount < 30; reqCount++)
		{
			var killNPCQuest = new KillNPCObjective(
				[
					NPCID.BlueSlime,
					NPCID.IceSlime,
					NPCID.SpikedJungleSlime,
			...
			{
				Assert.IsTrue(killNPCQuest.Progress == count / (float)reqCount);
				var npc = new NPC();
				npc.type = NPCID.BlueSlime;
				killNPCQuest.CountKill(npc);
			}

			Assert.IsTrue(killNPCQuest.Progress == 1f);
		}
	}
