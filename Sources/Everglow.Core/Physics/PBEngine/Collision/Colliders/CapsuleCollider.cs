...
namespace ChatGPT.Core.PBEngine.Collision.Colliders
{
    internal class CapsuleCollider : Collider2D
    {
        ...
        public override void GetContactInfo(in CollisionInfo info, float deltaTime, out List<CollisionEvent2D> collisionEvents)
        {
            ...
            if (info.Target.Collider is SphereCollider)
            {
                ...
                GeometryUtils.SphereCapsuleContactInfo(b.ParentObject.RigidBody.CentroidWorldSpace,
            ...
            {
                GetSegment(deltaTime, out Vector2 A, out Vector2 B);
                TileCollisionUtils.GetCapsuleTileContactInfo(GetAABB(deltaTime), A, B,_radius, ParentObject, info.Target, deltaTime, collisionEvents);
            }
        }
        ...
        public override bool TestCollisionCondition(Collider2D other, float deltaTime, out CollisionInfo info)
        {
            ...
            else if (other is TileCollider)
            {
                ...
                Vector2 normal;
                GetSegment(deltaTime, out Vector2 A, out Vector2 B);
                if (TileCollisionUtils.GetCapsuleTileCollisionInfo(GetAABB(deltaTime), A, B, _radius, out depth, out normal))
                {
                    info.Source = this.ParentObject;
