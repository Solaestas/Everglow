...
public abstract record Collider
{
	...
	public abstract bool Collision(Collider other);

	public static Collider Create(AABB rectangle) => new CRectangle(rectangle);
	public static Collider Create(Vector2 position, Vector2 size) => new CRectangle(new AABB(position, size));
}

file record CRectangle(AABB Rectangle) : Collider
{
	public override AABB AABB => Rectangle;

	public override bool Collision(Collider other)
	{
		if (!other.AABB.Intersect(Rectangle))
		{
			return false;
		}

		if (other is CRectangle)
		{
			return true;
		}
		return false;
...
// public override AABB AABB => new(circle.position - new Vector2(circle.radius, circle.radius), new
// Vector2(circle.radius * 2, circle.radius * 2)); public override bool Collision(Collider other,
// bool newCheck = true) { if (newCheck && !circle.Intersect(other.AABB)) return false;
...
// public CLine(Vector2 begin, Vector2 end) { this.begin = begin; this.end = end; }
...
// public CPoint(Vector2 pos) { this.pos = pos; }
...
// size += pos - value; pos = value; } }

// public Vector2 TopRight { get { return pos + new Vector2(size.X, 0); } set { if (value.X < pos.X
// || value.Y > pos.Y + size.Y) { throw new ArgumentException("TopRight set value is not correct",
// nameof(value)); }

// size.X += value.X - pos.X - size.X; size.Y += pos.Y - value.Y; pos = value - new Vector2(size.X,
// 0); } }

// public Vector2 BottomLeft { get { return pos + new Vector2(0, size.Y); } set { if (value.X >
// pos.X + size.X || value.Y < pos.Y) { throw new ArgumentException("BottomLeft set value is not
// correct", nameof(value)); }

// size.X += pos.X - value.X; size.Y += value.Y - pos.Y; pos = value - new Vector2(0, size.Y); } }

// public Vector2 BottomRight { get { return pos + size; } set { if (value.X < pos.X || value.Y <
// pos.Y) { throw new ArgumentException("BottomRight set value is not correct", nameof(value)); }

// size = value - pos; } } public Vector2 Size { get => size; set => size = value; } public Vector2
// Center { get { return pos + size / 2; } set { pos = value - size / 2; } }

// public Vertices Vertices => new Vertices() { pos, pos + new Vector2(size.X, 0), pos + new
// Vector2(0, size.Y), pos + size };

// public Rectangle Rectangle => new Rectangle((int)pos.X, (int)pos.Y, (int)size.X, (int)size.Y);
// public CRectangle AABB => this; public CRectangle() { } public CRectangle(float x, float y, float
// width, float height) { pos = new Vector2(x, y); size = new Vector2(width, height); } public
// CRectangle(Vector2 pos, Vector2 size) { this.pos = pos; this.size = size; } public bool
// Contain(Vector2 point) { return point.X > pos.X && point.X < pos.X + size.X && point.Y > pos.Y &&
// point.Y < pos.Y + size.Y; } public bool Colliding(ICollider collider) { if (collider is null) {
// return false; }

// if (!collider.AABB.Colliding(this)) { return false; }

// //矩形和矩形已经检验 if (collider is CRectangle) { return true; }

// if (collider is CCircle circle) { return Colliding(circle); }

// if (collider is CPolygon polygon) { return Colliding(polygon); }
...
// ///
// <returns> </returns>
// public bool Colliding(CRectangle rectangle) { if (rectangle is null) { return false; }

// if (rectangle.pos.X >= pos.X + size.X || rectangle.pos.X + rectangle.size.X <= pos.X ||
// rectangle.pos.Y >= pos.Y + size.Y || rectangle.pos.Y + rectangle.size.Y <= pos.Y) { return false; }

// return true; } public bool Colliding(CCircle circle) { if (circle is null) { return false; }
...
// public CCircle(Vector2 pos, float radius) { this.pos = pos; this.radius = radius; }

// public bool Colliding(ICollider collider) { if (collider is null) { return false; }

// if (!collider.AABB.Colliding(AABB)) { return false; }

// if (collider is CRectangle rectangle) { return rectangle.Colliding(this); }

// if (collider is CCircle circle) { return Colliding(circle); }

// if (collider is CPolygon polygon) { return Colliding(polygon); }

// return collider.Colliding(this); ; } public bool Colliding(CRectangle rectangle) { if (rectangle
// is null) { return false; }

// return rectangle.Colliding(this); } public bool Colliding(CCircle circle) { if (circle is null) {
// return false; }
...
// public CTriangle() { vertices = new Vertices(3); Initialize(); }
...
// public CLineSegment(Vector2 start, Vector2 end, float width) { Vector2 normal = (end -
// start).NormalLine(); vertices = new Vertices(start + normal * width, start - normal * width, end
// + normal * width, end - normal * width); this.start = start; this.end = end; this.width = width;
// Initialize(); }
...
// public CPolygon() { } public CPolygon(IEnumerable<Vector2> vertices) { this.vertices = new
// Vertices(vertices); Initialize(); } public CPolygon(params Vector2[] vertices) { this.vertices =
// new Vertices(vertices); Initialize(); } public Vector2 this[int index] { get => vertices[index];
// set { vertices[index] = value; Initialize(); } } public Vertices Vs => vertices; public Vector2[]
// Normals => normals; public Vector2[] Edges { get { Vector2[] edges = new Vector2[vertices.Count];
// for (int i = 0; i < vertices.Count; i++) { int next = (i + 1) % vertices.Count; edges[i] =
// vertices[next] - vertices[i]; } return edges; } } public void RemoveAt(int index) {
// vertices.RemoveAt(index); Initialize(); } public void Initialize() { if (!vertices.IsConvex()) {
// throw new Exception($"{this} is not convex"); }

// normals = new Vector2[vertices.Count]; for (int i = 0; i < vertices.Count; i++) { normals[i] =
// (vertices[i] - vertices[i + 1 == vertices.Count ? 0 : i + 1]).NormalLine(); }
...
// CRectangle aabb = colliders[0].AABB; for (int i = 1; i < colliders.Length; i++) { CRectangle rect
// = colliders[i].AABB; Vector2[] vs = new Vector2[] { aabb.TopLeft, aabb.BottomRight, rect.TopLeft,
// rect.BottomRight }; aabb = new CRectangle(vs.Min(), vs.Max()); } return aabb; } } public
// Colliders(int capacity) { colliders = new ICollider[capacity]; } public Colliders(ICollider[]
// colliders) { this.colliders = colliders; }
...
// for (int i = 0; i < Count; i++) { int next = i + 1 < Count ? i + 1 : 0; Vector2 edge = this[next]
// - this[i]; for (int j = 0; j < Count; j++) { if (i == j || next == j) { continue; }
