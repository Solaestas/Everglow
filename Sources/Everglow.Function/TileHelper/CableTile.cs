...
public abstract class CableTile : ModTile, ITileFluentlyDrawn
{
	...
	public virtual Rope ConnectRope(int i, int j, int i2, int j2)
	{
		...
		if (tile.TileType == Type)
		{
			...
			Rope rope = Rope.Create_Fixed_Head_and_Tail(new Vector2(i, j) * 16 + new Vector2(8), new Vector2(i2, j2) * 16 + new Vector2(8), counts, Elasticity, RopeUnitMass, LampDistance, SingleLampMass);
			return rope;
		}
		return null;
	}
