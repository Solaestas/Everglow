...
- 使用现有 MSTest 项目，并将自动化测试放在 `Sources/Everglow.UnitTests/Function/MissionSystem/` 的相应测试类或 `Objectives/` 子目录中。`WorldSide/Tests/` 与 `PlayerSide/Tests/` 是模组内示例/调试任务内容，不是 MSTest 测试目录。
