...
/// <summary>
/// 任务信息板<see cref="QuestContainer"/>
/// </summary>
public class UIQuestBlock : UIBlock
{
	/// <summary>
	/// 0: 描述(description)
	/// 1: 目标(objective)
	...
	{
		DrawQuestPanel(sb, Info.TotalHitBox, Color.White);
		int x = Info.TotalHitBox.X;
		int y = Info.TotalHitBox.Y;
		int w = Info.TotalHitBox.Width;
		int h = Info.TotalHitBox.Height;
		Texture2D tex_side = ModAsset.QuestMessageBoard_bottom_side.Value;
		sb.Draw(tex_side, new Rectangle(x + (w - 177) / 2, y + h - 13, 177, 13), null, Color.White);
		Texture2D tex_icon = ModAsset.QuestDescription_Icon.Value;
		if(QuestBlockStyle == 1)
		{
			tex_icon = ModAsset.QuestObjective_Icon.Value;
			Texture2D divider = ModAsset.QuestDetailPanelDivider.Value;
		...
		if (QuestBlockStyle == 2)
		{
			tex_icon = ModAsset.QuestWarning_Icon.Value;
		}
		if (QuestBlockStyle == 3)
		{
			tex_icon = ModAsset.QuestReward_Icon.Value;
		}
		sb.Draw(tex_icon, new Rectangle(x + 5, y + 6, 35, 35), null, Color.White);
	}

	public static void DrawQuestPanel(SpriteBatch sb, Rectangle hitbox, Color panelColor)
	{
		int x = hitbox.X;
		int y = hitbox.Y;
		int w = hitbox.Width;
		int h = hitbox.Height;
		Texture2D tex = ModAsset.QuestMessageBoard.Value;
