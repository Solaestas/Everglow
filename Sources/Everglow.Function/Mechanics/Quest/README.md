...
可能的原因有：任务计时结束、任务限制等。目标计时结束默认只冻结对应目标，不改变任务状态

### 任务目标

#### 目标类型

- 击杀NPC:击败一定数量的指定NPC（可指定多种类型，合计计数）, 多人游戏下计算所有玩家的击杀(未实现在特定条件下击杀, 因BOSS可能需要全伤害计算)
...
目标可通过 `WithTimeLimit` 设置独立时限。只有当前已激活目标消耗时间；同一更新时间片内先判定完成再推进计时，因此截止边界上的完成优先。目标超时后停留在当前位置并停止收集进度，不会自动令整个任务失败；重置或重试任务会同时重置目标计时器。任务级计时器继续独立存在。多人模式下，客户端或子服会在本地到期时先发送已积累差量；主服只接受抵达时仍属于当前活跃目标的贡献。
...
WorldSide 任务完成后在详情页导出“领取奖励”操作。物品奖励按玩家名分别领取，`RewardClaimedPlayers` 使用 Ordinal 精确比较并作为唯一领取记录：同名玩家共享记录，改名后视为新名称，任务完成后才加入的玩家也可领取。旧存档中的全局 `RewardClaimed` 布尔值不再参与判断。

单机直接记录本地玩家名并发奖。多人客户端只上传任务名；主世界服务器根据真实网络槽位从 `Netplay.Clients[whoAmI].Name` 取得玩家名并完成授权。玩家仍在主世界时由主服直接发奖；玩家位于子世界时，主服向所有子服发送包含槽位与预期名称的授权包，只有持有该活动槽位且名称一致的子服执行发奖。成功授权后主服广播任务快照，使所有世界覆盖同步领取名单。

领奖流程不维护 pending、确认、重试或补发。主服写入名单后玩家断线可能导致物品未交付，此时名单不回滚。
...
1. 主世界服务器(广播至所有客户端和所有子世界及其客户端): 主世界触发世界同步时(全任务同步)、任务生命周期改变(单任务同步)、差量数据上传后(单任务同步)、授权子世界发放奖励并同步领取名单
