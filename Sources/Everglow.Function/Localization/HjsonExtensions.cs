...
public static class HjsonExtensions
{
	...
	public static string ToFancyHjsonString(this JsonValue value, HjsonStyle? style = null)
	{
		...
		WriteFancyHjsonValue(stringWriter, value, 0, in usedStyle, noIndentation: true, isRootObject: false);
		...
		switch (value.JsonType)
		{
			case JsonType.Object:
				...
				if (commentedObject != null)
				{
					...
					string kwl = GetComments(commentedObject.Comments, string.Empty);
	...
	public static string GetRawString(this JsonValue value)
	{
		...
		{
			JsonType.String => ((string)value) ?? string.Empty,
	...
	{
		return comments.ContainsKey(key) ? GetComments(comments[key]) : string.Empty;
	}

	private static string GetComments(List<string> comments, int index)
	{
		return comments.Count > index ? GetComments(comments[index]) : string.Empty;
	}
	...
	private static T GetDelegateOfMethod<T>(string type, string methodName)
		where T : Delegate
	{
