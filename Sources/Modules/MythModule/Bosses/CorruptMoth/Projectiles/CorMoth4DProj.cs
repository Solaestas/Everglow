...
namespace Everglow.Sources.Modules.MythModule.Bosses.CorruptMoth.Projectiles
{
    public class CorMoth4DProj : ModProjectile
    {
        ...
        private Vector4 vecRotByYoZ(Vector4 vec,float rot)
        {
        ...
        Vector3 Projection(Vector4 vec, Vector2 center,float viewZ)
        {
            float k1 = -viewZ / (vec.W - viewZ);
            Vector3 v3 = new Vector3(vec.X, vec.Y,vec.Z);
            v3 = v3 + (k1 - 1) * (v3 - new Vector3(owner.Center, 0));
            return v3;
        }
        Vector2 Projection2(Vector3 v3, Vector2 center, out float scale,float viewZ)
        {
