namespace Everglow.Sources.Modules.MythModule.Bosses.Acytaea.Projectiles
{
    class AcytaeaTornado : ModProjectile
    {
        ...
        {
            if (WHOAMI >= 0 && Main.projectile[WHOAMI].active && Main.projectile[WHOAMI].type == Typ)
            {
                for (int z = -15; z < 16; z++)
                {
                    ...
                    for (int h = 0; h < 60; h++)
                    {
                        ...
                        Vx.Add(new VertexBase.CustomVertexInfo(Main.projectile[WHOAMI].Center - Main.screenPosition + new Vector2(0, z * 40), color3, new Vector3(0.5f, 1, 0)));
                    }
                    float ka = 1;
                    ...
                    Texture2D t = ModContent.Request<Texture2D>("MythMod/Projectiles/Acytaea/AcytaeaTornado").Value;
