...
{
    class AngelLight : ModProjectile
    {
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            for (int i = 0; i < Range; ++i)
            {
                ...
                if (i % 8 == 0)
                {
                    ...
                    Lighting.AddLight(Projectile.Center + v3 * (1 + width / 200f) / (float)(Math.Sqrt(ka)), k0, k0 * 0.7f, 0);
                }
                ...
                {
                    int num90 = Dust.NewDust(Projectile.Center + v3 * (1 + width / 200f) / (float)(Math.Sqrt(ka)), 0, 0, ModContent.DustType<MiscItems.Dusts.Fragrans.Fragrans>(), 0f, 0f, 0, default(Color), Main.rand.NextFloat(0.6f, 0.8f));
                    Main.dust[num90].noGravity = true;
                    Main.dust[num90].velocity = Projectile.velocity * 5;
                }
                if (!Main.gamePaused && Main.rand.Next(60 * (int)(dx + 2)) == 1)
                {
                    int num90 = Dust.NewDust(Projectile.Center + v3 * (1 + width / 200f) / (float)(Math.Sqrt(ka)), 0, 0, ModContent.DustType<MiscItems.Dusts.GoldShine>(), 0f, 0f, 0, default(Color), Main.rand.NextFloat(1.0f, 1.3f));
                    Main.dust[num90].noGravity = true;
                    Main.dust[num90].velocity = Projectile.velocity * 5;
                }
            }
