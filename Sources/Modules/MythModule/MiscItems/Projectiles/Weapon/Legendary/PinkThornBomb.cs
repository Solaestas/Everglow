...
namespace Everglow.Sources.Modules.MythModule.MiscItems.Projectiles.Weapon.Legendary
{
    public class PinkThornBomb : ModProjectile
    {
        ...
        {
            Texture2D Tex = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Projectiles/Weapon/Legendary/PinkThornBomb").Value;
            for (int i = 0; i < S * 8; i++)
            {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int i = 0; i < 90; i++)
            {
                ...
                int num5 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, ModContent.DustType<MiscItems.Dusts.PinkFog>(), 0f, 0f, 100, Color.White, (float)(4f * Math.Log10(Projectile.damage)));
            ...
            for (int i = 0; i < 60; i++)
            {
                ...
                int num5 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y) + v * 20f, Projectile.width, Projectile.height, ModContent.DustType<MiscItems.Dusts.PinkFog>(), 0f, 0f, 100, Color.White, (float)(12f * Math.Log10(Projectile.damage)));
            ...
            for (int t = 0; t < 16; t++)
            {
                ...
                int y = Projectile.NewProjectile(Projectile.InheritSource(Projectile), Projectile.Center + v2 * 3f, v2, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.FlowerPetalLight>(), Projectile.damage / 9, 0.5f, Main.myPlayer, 10f, 25f);
