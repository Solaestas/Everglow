...
namespace Everglow.Sources.Modules.MythModule.MiscItems.Projectiles.Weapon.Legendary
{
    public class TrapYoyo : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                if (Np[i] != -1 && Main.projectile[Np[i]].type == ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.TrapYoyoLight>() && Main.projectile[Np[i]].active)
                {
                    Cp += 1;
                }
                else
            ...
            for (int i = 0; i < 5; i++)
            {
                if (Np[i] != -1)
                {
                    if (Main.projectile[Np[i]].type == ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.TrapYoyoLight>())
                    {
            ...
            if (Ad2 % 90 == 0 && CryzyTime <= 0)
            {
                for (int i = 0; i < 5; i++)
                {
                    if (Np[i] == -1)
                    {
                        Np[i] = Projectile.NewProjectile(Projectile.InheritSource(Projectile), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.TrapYoyoLight>(), Projectile.damage * 2, 0.2f, Main.myPlayer, 0f, 0f);
                        break;
                    }
                }
            }
            ...
            if (CTime)
            {
                if (Ad2 % 2 == 0)
                {
                    ...
                    int num2 = Projectile.NewProjectile(Projectile.InheritSource(Projectile), Projectile.Center - v, v, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.AuDust>(), Projectile.damage / 3, 0.2f, Main.myPlayer, 5 + p.GetCritChance(DamageClass.Melee), 0f);
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            if (CTime)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    int num2 = Projectile.NewProjectile(Projectile.InheritSource(Projectile), Projectile.Center, v, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.AuDust>(), Projectile.damage / 3, 0.2f, Main.myPlayer, 0f, 0f);
        ...
        {
            Texture2D texture = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Projectiles/Weapon/Legendary/TrapYoyoGlow").Value;
            ...
            if (CTime)
            {
                Texture2D texture2 = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Projectiles/Weapon/Legendary/TrapYoyoLight").Value;
                for (int i = 0; i < 3; i++)
                {
