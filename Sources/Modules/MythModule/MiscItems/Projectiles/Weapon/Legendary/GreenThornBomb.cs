...
namespace Everglow.Sources.Modules.MythModule.MiscItems.Projectiles.Weapon.Legendary
{
    public class GreenThornBomb : ModProjectile
    {
        ...
        {
            Texture2D Tex = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Projectiles/Weapon/Legendary/GreenThornBomb").Value;
            for (int i = 0; i < S * 8; i++)
            {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int j = 0; j < 15; j++)
            {
                ...
                int zi = Projectile.NewProjectile(Projectile.InheritSource(Projectile), Projectile.Center + v * 3f, v, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.Cyanline>(), (int)((double)Projectile.damage) / 2, Projectile.knockBack, Projectile.owner, 0f, 0f);
            }
            for (int i = 0; i < 47; i++)
            {
            ...
            for (int i = 0; i < 90; i++)
            {
                ...
                int num5 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, ModContent.DustType<MiscItems.Dusts.Poison>(), 0f, 0f, 100, Color.White, (float)(4f * Math.Log10(Projectile.damage)));
            ...
            for (int i = 0; i < 60; i++)
            {
                ...
                int num5 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y) + v * 20f, Projectile.width, Projectile.height, ModContent.DustType<MiscItems.Dusts.Poison>(), 0f, 0f, 100, Color.White, (float)(12f * Math.Log10(Projectile.damage)));
