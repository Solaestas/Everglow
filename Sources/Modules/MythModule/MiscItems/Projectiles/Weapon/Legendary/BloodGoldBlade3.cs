...
{
    class BloodGoldBlade3 : ModProjectile
    {
        ...
        private Vector2 v_1 = new Vector2(-67, -14);
        ...
        public override void AI()
        {
            ...
            Vector2 v0 = v_1.RotatedBy(1.8 / 170d * Math.PI * (200 - Projectile.timeLeft));
            if (Projectile.timeLeft < 30)
            {
                Projectile.Kill();
                v0 = v_1.RotatedBy(1.8 * Math.PI);
            }
            Projectile.spriteDirection = Pdir;
            v0.X *= Pdir;
            Vector2 v1 = new Vector2(v0.X, v0.Y * 0.12f).RotatedBy(Prot) - new Vector2(29, 29);
            ...
            {
                Projectile.NewProjectile(Projectile.InheritSource(Projectile), player.Center + new Vector2(0, Main.rand.NextFloat(190f, 245f)).RotatedByRandom(Math.PI * 2d), Vector2.Zero, ModContent.ProjectileType<BloodDarkSmog>(), Projectile.damage * 2, Projectile.knockBack, player.whoAmI, 0, 0f);
            }
        }
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            if (ExtraKnife)
            {
                ...
                {
                    Vector2 v3 = v_1.RotatedBy(1.8 / 170d * Math.PI * (200 - Projectile.timeLeft - i)) * 1.5f;
                    if (Projectile.timeLeft < 30)
                    {
                        v3 = v_1.RotatedBy(1.8 * Math.PI) * 1.5f;
                    }
                    v3.X *= Pdir;
                    Vector2 v4 = new Vector2(v3.X, v3.Y * 0.12f).RotatedBy(Prot);
                    var normalDir = v_1.RotatedBy(1.8 / 170d * Math.PI * (200 - Projectile.timeLeft - i + 1)) * 1.5f - v_1.RotatedBy(1.8 / 170d * Math.PI * (200 - Projectile.timeLeft - i)) * 1.5f;
                    if (Projectile.timeLeft < 30)
                    {
                        if (i < 30 - Projectile.timeLeft)
                        {
                            normalDir = v_1.RotatedBy(1.8 / 170d * Math.PI * 170) * 1.5f - v_1.RotatedBy(1.8 / 170d * Math.PI * 169.99) * 1.5f;
                        }
                    }
                    normalDir.X *= Pdir;
