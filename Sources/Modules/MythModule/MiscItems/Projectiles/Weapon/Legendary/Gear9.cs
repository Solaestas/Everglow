    ...
    public class Gear9 : ModProjectile
    {
        ...
        public override void AI()
        {
            if (AimP == -1)
            {
                ...
                {
                    if (Main.projectile[d].type == ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.Gear3>())
                    {
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            for (int da = 0; da < 3; da++)
            {
                for (int db = 0; db < 3; db++)
                {
                    ...
                    t = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Projectiles/Weapon/Legendary/Gear11").Value;
            ...
            if (!Main.gamePaused)
            {
                ...
                if ((Acount + 0) % frequency == 0)
                {
                    ...
                    Projectile.NewProjectile(Projectile.InheritSource(Projectile), v0, v0s, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.LaserBullet>(), Projectile.damage, 1, Main.myPlayer, Projectile.ai[0], 0f);
                }
                if ((Acount + 50) % frequency == 0)
                {
                    Vector2 v1s = Main.MouseWorld - v1;
                    v1s = v1s / v1s.Length() * 22f;
                    Projectile.NewProjectile(Projectile.InheritSource(Projectile), v1, v1s, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.LaserBullet>(), Projectile.damage, 1, Main.myPlayer, Projectile.ai[0], 0f);
                }
                if ((Acount + 100) % frequency == 0)
                {
                    Vector2 v2s = Main.MouseWorld - v2;
                    v2s = v2s / v2s.Length() * 22f;
                    Projectile.NewProjectile(Projectile.InheritSource(Projectile), v2, v2s, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.LaserBullet>(), Projectile.damage, 1, Main.myPlayer, Projectile.ai[0], 0f);
                }
            }
            for (int d = 0; d < 9; d++)
            {
