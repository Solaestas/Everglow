...
{
    public class LaserWhip : ModProjectile
    {
        ...
        private void AI_165_Whip()
        {
            ...
            if (Utils.GetLerpValue(0.1f, 0.7f, t, true) * Utils.GetLerpValue(0.9f, 0.7f, t, true) > 0.5f && Main.rand.Next(3) != 0)
            {
                ...
                Dust dust = Dust.NewDustDirect(rectangle.TopLeft(), rectangle.Width, rectangle.Height, 182, 0f, 0f, 100, Color.White, 1f);
        ...
        private void DrawWhip(Projectile proj)
        {
            ...
            Texture2D value = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/UIimages/VisualTextures/RedWhip").Value;
            ...
            for (int i = 0; i < list.Count - 1; i++)
            {
                ...
                Microsoft.Xna.Framework.Color color = new Color(255, 255, 255, 0);
                ...
                Main.spriteBatch.Draw(LigEffect, value2 - Main.screenPosition, null, new Color(0.005f * vector2.Length(), 0.005f * vector2.Length(), 0.005f * vector2.Length(), 0), 0, new Vector2(56), 1f, SpriteEffects.None, 0f);
                for (int x = -3; x < 4; x++)
                {
                    for (int y = -3; y < 4; y++)
                    {
                ...
                for (int j = 0; j < 200; j++)
                {
                    if (!Hitj[j])
                    {
                        if ((Main.npc[j].Center - value2).Length() < 60 && !Main.npc[j].dontTakeDamage && !Main.npc[j].friendly && Main.npc[j].active && Collision.CanHit(value2, 1, 1, Main.npc[j].Center, 1, 1))
                        {
                            ...
                            int dam = (int)(Projectile.damage * Main.rand.NextFloat(0.85f, 1.15f));
                            bool cri = Main.rand.Next(100) < CritC;
                            Main.npc[j].StrikeNPC(dam, 8, Math.Sign(Projectile.velocity.X), cri);
                            ...
                            for (int z = 0; z < 40; z++)
                            {
                                ...
                                int h = Dust.NewDust(Main.npc[j].Center, 0, 0, 182, v4.X, v4.Y, 0, default(Color), Main.rand.NextFloat(1.5f, 3f));
                                Main.dust[h].noGravity = true;
                            }
                            for (int z = 0; z < 12; z++)
                            {
                                float num7 = (float)(Main.rand.Next(0, 2000) / 1000f * Math.PI);
                                if (Main.rand.NextBool(7) && PreC < 3)
                                {
                                    SoundEngine.PlaySound(SoundID.Item33, Projectile.Center);
                                    Vector2 v = new Vector2(0, 50).RotatedBy(Math.PI * z / 6d + num7);
                                    int num2 = Projectile.NewProjectile(Projectile.InheritSource(Projectile), Main.npc[j].Center, v, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.LaserWorm>(), Projectile.damage, 0.2f, Main.LocalPlayer.whoAmI, player.GetCritChance(DamageClass.Summon), Projectile.damage);
                                    Main.projectile[num2].timeLeft = 90;
                                    //Main.projectile[num2].owner = Main.LocalPlayer.whoAmI;
                                    player.AddBuff(ModContent.BuffType<MiscItems.Buffs.LaserWormBuff>(), 90);
                                    PreC += 1;
                                }
                            }
                            Common.MythContentGlobalNPC.LaserMark[j] = 180;
                            Common.MythContentGlobalNPC.LaserMark2[j] = 180;
                        }
                    }
                }
                value2 += vector2;
            }
            //value2 = DrawWhip_WhipSword(proj, list);
        }
