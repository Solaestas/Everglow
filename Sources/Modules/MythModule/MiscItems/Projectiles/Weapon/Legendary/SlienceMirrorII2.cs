...
{
    class SlienceMirrorII2 : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (ExtraKnife)
            {
                for (int k = 0; k < Projectile.timeLeft - 120; k++)
                {
                    if (k % 15 == 2)
                    {
                        Vector2 v3 = v_1.RotatedBy(1.2 / 170d * Math.PI * (310 - k) + 0.4) * 4.5f * (1 - (1 - Siort) / 2f);
                        if (Projectile.timeLeft < 30)
                        {
            ...
            if (ExtraKnife)
            {
                for (int i = 1; i < Maxz; ++i)
                {
                    ...
                    if (!Main.gamePaused)
                    {
                        ...
                        {
                            Projectile.NewProjectile(Projectile.InheritSource(Projectile), player.Center + v4 + normalDir, normalDir.RotatedBy(1.57) * -10f, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.SlienceMirrorIIBroken>(), Projectile.damage, 1, Main.myPlayer, Projectile.ai[0], 0f);
                        }
                    }
                }
            }
