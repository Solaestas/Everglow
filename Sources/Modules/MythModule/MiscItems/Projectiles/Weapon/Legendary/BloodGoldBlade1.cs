...
{
    class BloodGoldBlade1 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            Vector2 v0 = v_1.RotatedBy(4.6 / 170d * Math.PI * (200 - Projectile.timeLeft));
            if (Projectile.timeLeft < 30)
            {
                Projectile.Kill();
                v0 = v_1.RotatedBy(4.6 * Math.PI);
            }
            ...
            Vector2 v1 = new Vector2(v0.X, v0.Y * 0.27f).RotatedBy(Prot) * Sc - new Vector2(29, 29);
            ...
            {
                Projectile.NewProjectile(Projectile.InheritSource(Projectile), player.Center, (v1 + new Vector2(29, 29)) / 2f, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.BloodGold>(), Projectile.damage / 3 * 2, Projectile.knockBack, player.whoAmI, player.GetCritChance(DamageClass.Melee) + 8, 0f);
            }
            if (Projectile.timeLeft % 190 == 45)
            {
                Projectile.NewProjectile(Projectile.InheritSource(Projectile), player.Center + new Vector2(0, Main.rand.NextFloat(190f, 245f)).RotatedByRandom(Math.PI * 2d), Vector2.Zero, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Legendary.BloodDarkSmog>(), Projectile.damage * 2, Projectile.knockBack, player.whoAmI, 0f, 0f);
            }
        }
        public override bool PreDraw(ref Color lightColor)
        {
            Player player = Main.player[Projectile.owner];
            Texture2D t = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Projectiles/Weapon/Legendary/BloodGoldBlade").Value;
            ...
            if (ExtraKnife)
            {
                for (int k = 0; k < (Projectile.timeLeft - 25) * 2; k++)
                {
                    if (k % 5 == 2)
                    {
                        Vector2 v3 = v_1.RotatedBy(1.8 / 170d * Math.PI * (200 - Projectile.timeLeft - k)) * 1.5f;
                        if (Projectile.timeLeft < 30)
                        {
                            v3 = v_1.RotatedBy(4.6 * Math.PI) * 1.5f * Sc;
                        }
                        v3.X *= Pdir;
                        Vector2 v4 = new Vector2(v3.X, v3.Y * 0.27f).RotatedBy(Prot);
            ...
            else
            {
                for (int k = 0; k < Projectile.oldPos.Length; k++)
                {
                    ...
                    if (k % 10 == 2)
                    {
                        ...
                        if (Projectile.timeLeft < 30)
                        {
                            v3 = v_1.RotatedBy(1.8 * Math.PI) * 1.5f;
                        }
                        v3.X *= Pdir;
                        Vector2 v4 = new Vector2(v3.X, v3.Y * 0.12f).RotatedBy(Prot);
                        Vector2 v5 = new Vector2(v3.X, v3.Y).RotatedBy(Prot);
                        Vector2 drawPos = player.Center + v4 - Main.screenPosition + drawOrigin + new Vector2(1f, Projectile.gfxOffY) - new Vector2(29);
                        Color color = Lighting.GetColor((int)Projectile.Center.X / 16, (int)((double)Projectile.Center.Y / 16.0));
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            {
                int k = 2;
                Vector2 v3 = v_1.RotatedBy(4.6 / 170d * Math.PI * (200 - Projectile.timeLeft - k)) * 1.5f * Sc;
                if (Projectile.timeLeft < 30)
                {
                    v3 = v_1.RotatedBy(4.6 * Math.PI) * 1.5f * Sc;
                }
            ...
            if (ExtraKnife)
            {
                ...
                {
                    Vector2 v3 = v_1.RotatedBy(4.6 / 170d * Math.PI * (200 - Projectile.timeLeft - i)) * 1.5f * Sc;
                    if (Projectile.timeLeft < 30)
                    {
                        v3 = v_1.RotatedBy(4.6 * Math.PI) * 1.5f * Sc;
                    }
                    v3.X *= Pdir;
                    Vector2 v4 = new Vector2(v3.X, v3.Y * 0.27f).RotatedBy(Prot);
                    var normalDir = v_1.RotatedBy(4.6 / 170d * Math.PI * (200 - Projectile.timeLeft - i + 1)) * 1.5f * Sc - v_1.RotatedBy(4.6 / 170d * Math.PI * (200 - Projectile.timeLeft - i)) * 1.5f * Sc;
                    if (Projectile.timeLeft < 30)
                    {
                        if (i < 30 - Projectile.timeLeft)
                        {
                            normalDir = v_1.RotatedBy(4.6 / 170d * Math.PI * 170) * 1.5f * Sc - v_1.RotatedBy(4.6 / 170d * Math.PI * 169.99) * 1.5f * Sc;
                        }
                    }
                    normalDir.X *= Pdir;
