...
namespace Everglow.Sources.Modules.MythModule.MiscItems.Projectiles.Weapon.Fragrans
{
    class FragransBoomerang : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            if (Hit)
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    int num2 = Projectile.NewProjectile(Projectile.InheritSource(Projectile), Projectile.Center, v, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Fragrans.FragransBoomerangBrake>(), Projectile.damage / 3 * 2, 0.2f, Main.LocalPlayer.whoAmI, 0f, 0f);
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                Projectile.NewProjectileDirect(Projectile.InheritSource(Projectile), target.Center, velocity.RotatedByRandom(6.28) * Main.rand.NextFloat(0.85f, 1.15f), ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Fragrans.FragransMagicBeta>(), Projectile.damage / 2, knockback, player.whoAmI, ad * Main.rand.NextFloat(0.0f, 0.35f));
            }
            Projectile.NewProjectileDirect(Projectile.InheritSource(Projectile), target.Center, Vector2.Zero, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Fragrans.FragransBomb>(), Projectile.damage / 2, knockback, player.whoAmI, 0);
        }
        ...
        {
            Texture2D tg = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Projectiles/Weapon/Fragrans/FragransBoomerangGlow").Value;
