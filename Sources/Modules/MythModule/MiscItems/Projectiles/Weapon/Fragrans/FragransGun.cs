            ...
            if (Tokill < 0)
            {
                for (int j = 0; j < 200; j++)
                {
                    if ((Main.npc[j].Center - Projectile.Center).Length() < 30 && !Main.npc[j].dontTakeDamage && !Main.npc[j].friendly && Main.npc[j].life > 0)
                    {
                        ...
                        for (int y = 0; y < 8; y++)
                        {
                            int num90 = Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y) + new Vector2(0, Main.rand.NextFloat(48f)).RotatedByRandom(3.1415926), 0, 0, ModContent.DustType<MiscItems.Dusts.Fragrans.Fragrans3>(), 0f, 0f, 100, default(Color), Main.rand.NextFloat(1.0f, 2.7f));
