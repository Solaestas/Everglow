...
{
	public class FragransMagicMoon : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            K = Main.rand.NextFloat(0, (float)MathHelper.TwoPi);
            for (int j = 0; j < 6; j++)
            {
                Vector2 v = new Vector2(0, 10).RotatedBy(Math.PI * j / 3d + K);
                Projectile.NewProjectile(Projectile.InheritSource(Projectile), Projectile.Center, v, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Fragrans.FragransMagicMoon2>(), Projectile.damage / 2, 1, Main.myPlayer, Projectile.ai[0], 0f);
            }
        }
        ...
        {
            Projectile.rotation += (Projectile.velocity.Length() / 100f);
