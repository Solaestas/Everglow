...
{
    public class FixCoinDefense5 : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int x = X0; x < 58; x++)
            {
                if (player.inventory[x].accessory)
                {
                    ...
                    {
                        ////player.inventory[x].prefix = ModContent.PrefixType<Prefixs.Bedrocky1>();
                    }
                    else
                    {
                        if (Main.rand.NextBool(2))
                        {
                            //player.inventory[x].prefix = ModContent.PrefixType<Prefixs.Bedrocky2>();
                        }
                        else
                        {
                            //player.inventory[x].prefix = ModContent.PrefixType<Prefixs.Bedrocky3>();
                        }
                    }
                    ...
                    CombatText.NewText(new Rectangle((int)player.Center.X - 10, (int)player.Center.Y - 10, 20, 20), Color.Orange, tex1 + player.inventory[x].Name + tex2);
                    return;
                }
            ...
            for (int x = X0; x >= 0; x--)
            {
                if (player.inventory[x].accessory)
                {
                    if (Main.rand.NextBool(2))
                    {
                        ////player.inventory[x].prefix = ModContent.PrefixType<Prefixs.Bedrocky1>(); //not implemented yet
                    }
                    else
                    {
                        if (Main.rand.NextBool(2))
            ...
            Item.NewItem(null, Projectile.Center, ModContent.ItemType<MiscItems.FixCoins.FixCoinDefense5>());
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            Main.spriteBatch.Draw(LightE, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), null, new Color(0.3f * Stre * Stre, 0.21f * Stre * Stre, 0, 0), -(float)(Math.Sin(Main.time / 26d)) + 0.6f, new Vector2(128f, 128f), 0.5f + (float)(0.25 * Math.Sin(Main.time / 26d)), SpriteEffects.None, 0);
            Main.spriteBatch.Draw(LightE, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), null, new Color(1f * Stre * Stre, 0.7f * Stre * Stre, 0, 0), (float)(Math.Sin(Main.time / 12d + 2)) + 1.6f, new Vector2(128f, 128f), 0.5f + (float)(0.25 * Math.Sin(Main.time / 26d)), SpriteEffects.None, 0);
            Main.spriteBatch.Draw(LightE, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), null, new Color(0.3f * Stre * Stre, 0.21f * Stre * Stre, 0, 0), (float)Math.PI / 2f + (float)(Main.time / 9d), new Vector2(128f, 128f), 0.5f + (float)(0.25 * Math.Sin(Main.time / 26d + 1.57)), SpriteEffects.None, 0);
            Main.spriteBatch.Draw(LightE, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), null, new Color(1f * Stre * Stre, 0.7f * Stre * Stre, 0, 0), (float)(Main.time / 26d), new Vector2(128f, 128f), 0.5f + (float)(0.25 * Math.Sin(Main.time / 26d + 3.14)), SpriteEffects.None, 0);
            Main.spriteBatch.Draw(LightE, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), null, new Color(1f * Stre * Stre, 0.7f * Stre * Stre, 0, 0), -(float)(Main.time / 26d), new Vector2(128f, 128f), 0.5f + (float)(0.25 * Math.Sin(Main.time / 26d + 4.71)), SpriteEffects.None, 0);
            Texture2D Ball = (Texture2D)ModContent.Request<Texture2D>(Texture);
            Texture2D Circle = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Projectiles/Typeless/FixCoinFramework").Value;
            Texture2D Light = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Projectiles/Typeless/FixCoinLight5").Value;
