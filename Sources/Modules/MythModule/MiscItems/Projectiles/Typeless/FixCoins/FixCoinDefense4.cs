...
{
    public class FixCoinDefense4 : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int h = 0; h < 20; h++)
            {
                ...
                int r = Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y) - new Vector2(4, 4), 0, 0, ModContent.DustType<TheTusk.Dusts.PurePurple>(), 0, 0, 0, default, 15f * Main.rand.NextFloat(0.4f, 1.1f));
            ...
            for (int x = X0; x < 58; x++)
            {
                if (player.inventory[x].accessory)
                {
                    ...
                    {
                        //player.inventory[x].prefix = ModContent.PrefixType<Prefixs.Crystalized1>();
                    }
                    else
                    {
                        //player.inventory[x].prefix = ModContent.PrefixType<Prefixs.Crystalized2>();
                    }
                    ...
                    for (int h = 0; h < 20; h++)
                    {
                        ...
                        int r = Dust.NewDust(new Vector2(player.Center.X, player.Center.Y) - new Vector2(4, 4), 0, 0, ModContent.DustType<TheTusk.Dusts.PurePurple>(), 0, 0, 0, default, 15f * Main.rand.NextFloat(0.4f, 1.1f));
                        Main.dust[r].noGravity = true;
                        Main.dust[r].velocity = v3;
                    ...
                    CombatText.NewText(new Rectangle((int)player.Center.X - 10, (int)player.Center.Y - 10, 20, 20), Color.Purple, tex1 + player.inventory[x].Name + tex2);
                    return;
                }
            ...
            Item.NewItem(null, Projectile.Center, ModContent.ItemType<MiscItems.FixCoins.FixCoinDefense4>());
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            Texture2D Light = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Projectiles/Typeless/FixCoinLight4").Value;
