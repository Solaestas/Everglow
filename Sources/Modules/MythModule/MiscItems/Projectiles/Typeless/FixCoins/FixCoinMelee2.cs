...
{
    public class FixCoinMelee2 : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int h = 0; h < 20; h++)
            {
                ...
                int r = Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y) - new Vector2(4, 4), 0, 0, ModContent.DustType<TheTusk.Dusts.PureLime>(), 0, 0, 0, default, 15f * Main.rand.NextFloat(0.4f, 1.1f));
            ...
            for (int h = 0; h < 5; h++)
            {
                ...
                //Gore.NewGore(null, Projectile.position, vF, ModContent.Find<ModGore>("Everglow/CoinFrame1").Type, 100f);
            }
            ...
            for (int x = X0; x < 58; x++)
            {
                if (player.inventory[x].accessory)
                {
                    ...
                    {
                        player.inventory[x].prefix = PrefixID.Rash;
                    }
                    else
                    {
                        player.inventory[x].prefix = PrefixID.Intrepid;
                    }
                    ...
                    for (int h = 0; h < 20; h++)
                    {
                        ...
                        int r = Dust.NewDust(new Vector2(player.Center.X, player.Center.Y) - new Vector2(4, 4), 0, 0, ModContent.DustType<TheTusk.Dusts.PureLime>(), 0, 0, 0, default, 15f * Main.rand.NextFloat(0.4f, 1.1f));
                        Main.dust[r].noGravity = true;
                        Main.dust[r].velocity = v3;
                    ...
                    CombatText.NewText(new Rectangle((int)player.Center.X - 10, (int)player.Center.Y - 10, 20, 20), Color.Lime, tex1 + player.inventory[x].Name + tex2);
                    return;
                }
            ...
            Item.NewItem(null, Projectile.Center, ModContent.ItemType<MiscItems.FixCoins.FixCoinMelee2>());
        ...
        public override void PostDraw(Color lightColor)
        {
            for (int j = 0; j < 3; j++)
            {
                ...
                if (bars.Count > 2)
                {
                    ...
                    Texture2D Blue = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/UIimages/VisualTextures/heatmapLime").Value;
            ...
            Main.spriteBatch.Draw(LightE, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), null, new Color(0, 0.15f * Stre * Stre, 0, 0), -(float)(Math.Sin(Main.time / 26d)) + 0.6f, new Vector2(128f, 128f), 0.5f + (float)(0.25 * Math.Sin(Main.time / 26d)), SpriteEffects.None, 0);
            Main.spriteBatch.Draw(LightE, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), null, new Color(0, 0.5f * Stre * Stre, 0, 0), (float)(Math.Sin(Main.time / 12d + 2)) + 1.6f, new Vector2(128f, 128f), 0.5f + (float)(0.25 * Math.Sin(Main.time / 26d)), SpriteEffects.None, 0);
            Main.spriteBatch.Draw(LightE, Projectile.Center - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), null, new Color(0, 0.15f * Stre * Stre, 0, 0), (float)Math.PI / 2f + (float)(Main.time / 9d), new Vector2(128f, 128f), 0.5f + (float)(0.25 * Math.Sin(Main.time / 26d + 1.57)), SpriteEffects.None, 0);
            ...
            Texture2D Light = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Projectiles/Typeless/FixCoinLight2").Value;
