...
namespace Everglow.Sources.Modules.MythModule.MiscItems.Tiles
{
    public class FragransStand : ModTile
    {
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            for (int x = 0; x < 2; x++)
            {
                ...
                {
                    Texture2D value = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Projectiles/Typeless/Moon").Value;
                    ...
                    for (int z = 0; z < 4; z++)
                    {
                        ...
                        Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Projectiles/Typeless/Cloud" + z.ToString()).Value, value3 - Main.screenPosition + zero + vCl[z], null, new Color((int)(255 * Col[z]), (int)(255 * Col[z]), (int)(255 * Col[z]), 0), 0f, origin, 2f, effects, 0f);
                    }
                }
            }
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            if (Value < 800 + addP + addO)
            {
                if (Value < 60 + addO)
                {
                    Projectile.NewProjectile(null, new Vector2(i * 16f, j * 16f) - v, v / 100f, ModContent.ProjectileType<MiscItems.Projectiles.Typeless.OrangeStar>(), 0, 0, Main.LocalPlayer.whoAmI, i * 16f, j * 16f);
                    NoO = 0;
                }
                else
                {
                    Projectile.NewProjectile(null, new Vector2(i * 16f, j * 16f) - v, v / 100f, ModContent.ProjectileType<MiscItems.Projectiles.Typeless.PurpleStar>(), 0, 0, Main.LocalPlayer.whoAmI, i * 16f, j * 16f);
                    NoO++;
                    NoP = 0;
                }
            }
            ...
            {
                Projectile.NewProjectile(null, new Vector2(i * 16f, j * 16f) - v, v / 100f, ModContent.ProjectileType<MiscItems.Projectiles.Typeless.BlueStar>(), 0, 0, Main.LocalPlayer.whoAmI, i * 16f, j * 16f);
                NoP++;
                NoO++;
            }
