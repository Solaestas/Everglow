    ...
    public abstract class GemAmmo : SlingshotAmmo
    {
        ...
        public override void AI()
        {
            ...
            if (TimeTokill < 0)
            {
                ...
                Dust.NewDustDirect(Projectile.Center - new Vector2(4, 2)/*Half Dust Size*/, 0, 0, dustType, 0, 0, 0, default, Main.rand.NextFloat(0.85f, 1.15f));
            }
            else
            {
                if (TimeTokill < 10)
        ...
        public override void DrawTrail()
        {
            ...
            for (int i = 1; i < Projectile.oldPos.Length; ++i)
            {
                ...
                if(fac1 > fac2)
                {
                    bars.Add(new Vertex2D(Projectile.oldPos[i] + normalDir * -width + new Vector2(10, 10) - Main.screenPosition + Projectile.velocity, color, new Vector3(fac1, 0, 0)));
                    bars.Add(new Vertex2D(Projectile.oldPos[i] + normalDir * width + new Vector2(10, 10) - Main.screenPosition + Projectile.velocity, color, new Vector3(fac1, 1, 0)));
                    if(i < Projectile.oldPos.Length - 1)
                    {
                        ...
                        normalDir = Utils.SafeNormalize(new Vector2(-normalDir.Y, normalDir.X), Vector2.Zero);
                        bars.Add(new Vertex2D(Projectile.oldPos[i + 1] * (1 - fac3) + Projectile.oldPos[i] * fac3 + normalDir * -width + new Vector2(10, 10) - Main.screenPosition + Projectile.velocity, color, new Vector3(1, 0, 0)));
                        bars.Add(new Vertex2D(Projectile.oldPos[i + 1] * (1 - fac3) + Projectile.oldPos[i] * fac3 + normalDir * width + new Vector2(10, 10) - Main.screenPosition + Projectile.velocity, color, new Vector3(1, 1, 0)));

                        bars.Add(new Vertex2D(Projectile.oldPos[i + 1] * (1 - fac3) + Projectile.oldPos[i] * fac3 + normalDir * -width + new Vector2(10, 10) - Main.screenPosition + Projectile.velocity, color, new Vector3(0, 0, 0)));
                        bars.Add(new Vertex2D(Projectile.oldPos[i + 1] * (1 - fac3) + Projectile.oldPos[i] * fac3 + normalDir * width + new Vector2(10, 10) - Main.screenPosition + Projectile.velocity, color, new Vector3(0, 1, 0)));
                    }
                }
                else
                {
