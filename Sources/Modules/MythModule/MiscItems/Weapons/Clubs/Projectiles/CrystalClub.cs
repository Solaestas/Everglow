...
{
    public class CrystalClub : ModProjectile
    {

        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("CrystalClub");
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft < 22 && Main.mouseLeft && !p.dead && p.HeldItem.type == ModContent.ItemType<Clubs.CrystalClub>())
            {
            ...
            /*int num25 = Dust.NewDust(Projectile.Center - new Vector2(4, 4) + new Vector2(li, li).RotatedBy(Projectile.rotation), 0, 0, 181, 0, 0, 0, default(Color), 1.8f);
            ...
            if (lz % 10 == 1 && Ome >= 0.39)
            {
                ...
                vk = vk / vk.Length() * 11f;
                int i = Projectile.NewProjectile(Projectile.InheritSource(Projectile), p.Center + vk, vk, 94, Projectile.damage, Projectile.knockBack * 0.5f, p.whoAmI, 0, 0);
            ...
            if (lz % 30 == 16 && Ome >= 0.39)
            {
                ...
                vk = vk / vk.Length() * 12f;
                int i = Projectile.NewProjectile(Projectile.InheritSource(Projectile), p.Center + vk, vk, ModContent.ProjectileType<Projectiles.CrystalClub2>(), Projectile.damage, Projectile.knockBack * 0.5f, p.whoAmI, p.GetCritChance(DamageClass.Melee), 0);
            }
        }
