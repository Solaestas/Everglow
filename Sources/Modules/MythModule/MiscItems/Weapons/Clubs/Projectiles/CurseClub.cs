...
namespace Everglow.Sources.Modules.MythModule.MiscItems.Weapons.Clubs.Projectiles
{
    public class CurseClub : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft < 22 && Main.mouseLeft && !p.dead && p.HeldItem.type == ModContent.ItemType<Clubs.CurseClub>())
            {
            ...
            ef4.Parameters["uImage1"].SetValue(ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/UIimages/VisualTextures/ClubShaderCFlame").Value);
            ...
            ef5.Parameters["uImage1"].SetValue(ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/UIimages/VisualTextures/ClubShaderCFlame").Value);
            ...
            /*int num25 = Dust.NewDust(Projectile.Center - new Vector2(4,4) + new Vector2(li,li).RotatedBy(Projectile.rotation), 0, 0, 75, 0, 0, 0, default(Color), 1.8f);
            ...
            if (lz % 10 == 1 && Ome >= 0.39)
            {
                ...
                vk = vk / vk.Length() * 11f;
                int i = Projectile.NewProjectile(Projectile.InheritSource(Projectile), p.Center + vk, vk, 96, Projectile.damage, Projectile.knockBack * 0.5f, p.whoAmI, 0, 0);
            ...
            if (lz % 30 == 16 && Ome >= 0.39)
            {
                ...
                vk = vk / vk.Length() * 12f;
                int i = Projectile.NewProjectile(Projectile.InheritSource(Projectile), p.Center + vk, vk, ModContent.ProjectileType<Projectiles.CurseClub2>(), Projectile.damage, Projectile.knockBack * 0.5f, p.whoAmI, p.GetCritChance(DamageClass.Melee), 0);
            }
            p.ChangeDir(Projectile.direction);
            p.heldProj = Projectile.whoAmI;
        }
        ...
        {
            SpriteEffects effects = SpriteEffects.None;
            Texture2D texture = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscItems/Weapons/Clubs/Projectiles/CurseClubGlow").Value;
