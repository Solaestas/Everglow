...
{
    public class FragransMagic : ModItem
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Moon & Fragrans Magic");
            DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "金堆作浪");
		...
		{
			return File.Exists(Main.WorldPath + Main.LocalPlayer.name + "MoonFraMagic.json");
		}
			...
			for (int x = -10; x < 11; x++)
			{
				for (int y = -10; y < 11; y++)
				{
					if (Main.tile[x + (int)(player.Center.X / 16f), y + (int)(player.Center.Y / 16f)].TileType == ModContent.TileType<Tiles.MoonFragrans>())
					{
						...
						{
							File.Create(Main.WorldPath + Main.LocalPlayer.name + "MoonFraMagic.json").Close();
						}
						x = 15;
						break;
					}
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            {
                float ad = 0.3f;
                Projectile.NewProjectileDirect(source, position, velocity.RotatedByRandom(0.3) * Main.rand.NextFloat(0.85f, 1.15f), ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Fragrans.FragransMagicBeta>(), (int)Dam, knockback, player.whoAmI, ad * Main.rand.NextFloat(0.0f, 0.35f));
            }
            Projectile.NewProjectileDirect(source, position, velocity.RotatedByRandom(0.003) * Main.rand.NextFloat(0.85f, 1.15f) * 3, ModContent.ProjectileType<MiscItems.Projectiles.Weapon.Fragrans.FragransMagicMoon>(), (int)Dam * 2, knockback, player.whoAmI, 0);
            return false;
        }
