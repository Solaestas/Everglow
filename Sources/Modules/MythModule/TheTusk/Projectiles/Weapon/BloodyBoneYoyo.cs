            ...
            if (hit >= 4)
            {
                ...
                for (int j = 0; j < 5; j++)
                {
                    for (int z = 0; z < 4; z++)
                    {
                        ...
                        int dus = Dust.NewDust(new Vector2(target.position.X, target.position.Y), target.width, target.height, 183, v2.X, v2.Y, 100, default(Color), 0.4f);
                    ...
                    int r = Dust.NewDust(new Vector2(target.Center.X, target.Center.Y) - new Vector2(4, 4) + v1, 0, 0, 183, v3.X, v3.Y, 0, default(Color), 2f);
                ...
                for (int i = 0; i < 10; i++)
                {
                    ...
                    Dust.NewDust(target.position, target.width, target.height, 183, v.X, v.Y, 150, default(Color), Main.rand.NextFloat(0.5f, 1.2f));
                }
                hit = 0;
            }
            for (int y = 0; y < 16; y++)
            {
                int num90 = Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y) - new Vector2(4, 4) + Projectile.velocity / Projectile.velocity.Length() * 25, 4, 4, DustID.Blood, 0f, 0f, 100, default(Color), Main.rand.NextFloat(1.3f, 2f));
