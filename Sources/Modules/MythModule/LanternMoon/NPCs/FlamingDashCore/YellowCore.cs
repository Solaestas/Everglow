...
{
    //[AutoloadBossHead]
    public class YellowCore : ModNPC
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Yellow Core");
            DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "黄焰核心");
        ...
        float x = 0;
        float Sca = 0;
        Color Aimcolor = new Color(255, 204, 0, 0);
            ...
            if (NPC.life <= 0)
            {
                ...
                for (int d = 0; d < Main.player.Length; d++)
                {
                    if (Main.player[d].active && !Main.player[d].dead)
                    {
                        ...
                        Main.player[d].AddBuff(ModContent.BuffType<Buffs.YellowImmune>(), 1800);
