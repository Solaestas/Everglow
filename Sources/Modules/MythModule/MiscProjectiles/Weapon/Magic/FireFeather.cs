...
namespace Everglow.Sources.Modules.MythModule.MiscProjectiles.Weapon.Magic
{
    public class FireFeather : ModProjectile
    {
        ...
        {
            DisplayName.SetDefault("FireFeather");
            DisplayName.AddTranslation((int)GameCulture.CultureName.Chinese, "火羽");
        }
        ...
        public override void AI()
        {
            ...
            int num90 = Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y) - new Vector2(12, 12) - Projectile.velocity, 16, 16, 6, 0f, 0f, 100, default(Color), Main.rand.NextFloat(1f, 2.6f));
            ...
            int r = Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y) - new Vector2(4, 4) + Projectile.velocity / Projectile.velocity.Length() * 12f, 0, 0, ModContent.DustType<LanternMoon.Dusts.Flame2>(), 0, 0, 0, default(Color), 4f);
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int j = 0; j < 200; j++)
            {
                if ((Main.npc[j].Center - Projectile.Center).Length() < 64 && !Main.npc[j].dontTakeDamage && !Main.npc[j].friendly)
                {
                    ...
                    Main.npc[j].StrikeNPC(Dam, 2, Math.Sign(Projectile.velocity.X), Main.rand.Next(100) < Projectile.ai[0]);
                    Player player = Main.player[Projectile.owner];
                    player.addDPS((int)(Dam * (1 + Projectile.ai[0] / 100f)));
                }
            }
            for (int i = 0; i < 5; i++)
            {
