    ...
    public class ThunderBall : ModProjectile
    {
        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
            ...
            for (int y = 0; y < 3; y++)
            {
                ...
                int h = Projectile.NewProjectile(null, Projectile.Center - v, v, ModContent.ProjectileType<MiscProjectiles.Weapon.Magic.ThunderBall2>(), Projectile.damage / 3, Projectile.knockBack, player.whoAmI, player.GetCritChance(DamageClass.Magic) * 100 + 16);
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            {
                Vector2 v = Projectile.velocity.RotatedBy(Math.PI * y / 1.5 + a) / Projectile.velocity.Length() * 15f;
                int h = Projectile.NewProjectile(null, Projectile.Center, v, ModContent.ProjectileType<MiscProjectiles.Weapon.Magic.ThunderBall2>(), Projectile.damage / 3, Projectile.knockBack, player.whoAmI);
                Main.projectile[h].timeLeft = Main.rand.Next(4, 16);
            }
            for (int θ = 0; θ < 40; θ++)
            {
            ...
            if (TotalPower > 0)
            {
                for (int j = 0; j < 200; j++)
                {
                    if (!HasBeenHit[j] && (Main.npc[j].Center - (Projectile.Center + Projectile.velocity * 10f)).Length() < 100 && !Main.npc[j].dontTakeDamage && !Main.npc[j].friendly && Main.npc[j].active)
                    {
                        ...
                        HasBeenHit[j] = true;
                        TotalPower--;
                        Projectile.NewProjectile(Projectile.InheritSource(Projectile), Main.npc[j].Center, Vector2.Zero, ModContent.ProjectileType<MiscProjectiles.Weapon.Magic.ThunderBallToNPC>(), Projectile.damage / 3, Projectile.knockBack, Projectile.owner, Projectile.whoAmI, j);
                    }
