...
{
    public class HepuyuanDown : ModProjectile
    {
        ...
        {
            DisplayName.SetDefault("He pu yuan");
        ...
        public override bool PreAI()
        {
            ...
            {
                Vector2 v0 = new Vector2(Main.rand.NextFloat(0.8f, 15f), 0).RotatedByRandom(6.28);
                if (Main.rand.Next(2) == 0)
                {
                    Gore.NewGore(null, OldplCen[0], Projectile.velocity * 0.35f + v0, ModContent.GoreType<XiaoDash0>(), Main.rand.NextFloat(0.65f, 1.5f));
                }
                else
                {
                    Gore.NewGore(null, OldplCen[0], Projectile.velocity * 0.35f + v0, ModContent.GoreType<XiaoDash1>(), Main.rand.NextFloat(0.65f, 1.5f));
                }
            }
            kad++;
            ...
            if (Collision.SolidCollision(Projectile.Center + Projectile.velocity, 1, 1))
            {
                ...
                if (!Crash)
                {
                    for (int f = 0; f < 25; f++)
                    {
                        ...
                        if (Main.rand.Next(2) == 0)
                        {
                            Gore.NewGore(null, Pos, Dy, ModContent.GoreType<XiaoDash0>(), Main.rand.NextFloat(1f, 1.5f));
                        }
                        else
                        {
                            Gore.NewGore(null, Pos, Dy, ModContent.GoreType<XiaoDash1>(), Main.rand.NextFloat(1f, 1.5f));
                        }
                    }
                    for (int f = 0; f < 8; f++)
                    {
                        ...
                        int h = Projectile.NewProjectile(Projectile.InheritSource(Projectile), Pos, Vector2.Zero, ModContent.ProjectileType<MiscProjectiles.Weapon.Melee.Hepuyuan.HepuyuanSpice>(), Projectile.damage, Projectile.knockBack, player.whoAmI, Main.rand.NextFloat(50f, 110f), 0);
                    ...
                    for (int f = 0; f < 12; f++)
                    {
                        ...
                        int h = Projectile.NewProjectile(Projectile.InheritSource(Projectile), Pos, Vector2.Zero, ModContent.ProjectileType<MiscProjectiles.Weapon.Melee.Hepuyuan.HepuyuanShake>(), 0, Projectile.knockBack, player.whoAmI, Main.rand.NextFloat(17f, 32f) * (Math.Abs(f - 5.5f) + 0.5f), 0);
