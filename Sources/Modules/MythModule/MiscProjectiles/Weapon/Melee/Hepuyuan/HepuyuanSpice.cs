        ...
        private void DrawXiao(On.Terraria.Main.orig_DoDraw_Tiles_NonSolid orig, Terraria.Main self)
        {
            for (int p = 0; p < Main.projectile.Length; p++)
            {
                if (Main.projectile[p].type == ModContent.ProjectileType<HepuyuanShake>())
                {
                    ...
                    float Stre = Math.Clamp((Main.projectile[p].timeLeft - 60) / 60f, 0f, 1f);
                    ...
                    Texture2D t = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Melee/Hepuyuan/HepuyuanShake").Value;
                ...
                if (Main.projectile[p].type == ModContent.ProjectileType<HepuyuanSpice>())
                {
                    ...
                    Texture2D t = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Melee/Hepuyuan/HepuyuanSpice").Value;
                    Main.graphics.GraphicsDevice.Textures[0] = t;
                    Main.graphics.GraphicsDevice.DrawUserPrimitives(PrimitiveType.TriangleList, Vx.ToArray(), 0, Vx.Count / 3);
                    Main.spriteBatch.End();
                    Main.spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.Default, RasterizerState.CullNone, null, Main.GameViewMatrix.TransformationMatrix);
