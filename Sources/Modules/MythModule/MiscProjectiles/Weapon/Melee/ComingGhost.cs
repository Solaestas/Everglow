...
namespace Everglow.Sources.Modules.MythModule.MiscProjectiles.Weapon.Melee
{
    class ComingGhost : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            Projectile.NewProjectile(null, target.Center + v, -v * 2.5f, ModContent.ProjectileType<MiscProjectiles.Weapon.Melee.GhostHit>(), Projectile.damage, Projectile.knockBack, player.whoAmI, v2.X, v2.Y);
            Projectile.NewProjectile(null, target.Center + v + v.RotatedBy(Math.PI / 2d) * 0.05f, -v * 2.42f, ModContent.ProjectileType<MiscProjectiles.Weapon.Melee.GhostHit>(), Projectile.damage, Projectile.knockBack, player.whoAmI, v2.X, v2.Y);
            Projectile.NewProjectile(null, target.Center + v - v.RotatedBy(Math.PI / 2d) * 0.05f, -v * 2.34f, ModContent.ProjectileType<MiscProjectiles.Weapon.Melee.GhostHit>(), Projectile.damage, Projectile.knockBack, player.whoAmI, v2.X, v2.Y);
        }
        float ka = 0;
                ...
                for (int j = 0; j < 200; j++)
                {
                    if ((Main.npc[j].Center - Projectile.Center).Length() < 510 && !Main.npc[j].dontTakeDamage && !Main.npc[j].friendly && Main.npc[j].active && Main.npc[j].CanBeChasedBy())
                    {
                        if (Main.rand.Next(7) == 1)
                        {
                            Vector2 v = new Vector2(0, 80).RotatedByRandom(Math.PI * 2) * 5f;
                            Vector2 v2 = new Vector2(0, 0.5f).RotatedByRandom(Math.PI * 2);
                            Projectile.NewProjectile(null, Main.npc[j].Center + v, -v * 2.5f, ModContent.ProjectileType<MiscProjectiles.Weapon.Melee.GhostHit>(), Projectile.damage, Projectile.knockBack, player.whoAmI, v2.X, v2.Y);
                            Projectile.NewProjectile(null, Main.npc[j].Center + v + v.RotatedBy(Math.PI / 2d) * 0.05f, -v * 2.42f, ModContent.ProjectileType<MiscProjectiles.Weapon.Melee.GhostHit>(), Projectile.damage, Projectile.knockBack, player.whoAmI, v2.X, v2.Y);
                            Projectile.NewProjectile(null, Main.npc[j].Center + v - v.RotatedBy(Math.PI / 2d) * 0.05f, -v * 2.34f, ModContent.ProjectileType<MiscProjectiles.Weapon.Melee.GhostHit>(), Projectile.damage, Projectile.knockBack, player.whoAmI, v2.X, v2.Y);
                            times++;
                            if (times >= 3)
                            {
                                break;
        ...
        {
            Player player = Main.player[Projectile.owner];
            Texture2D t = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Melee/ComingGhost").Value;
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            if (ExtraKnife)
            {
                for (int i = 1; i < (Projectile.timeLeft - 25) * 2; ++i)
                {
                    ...
                    bart.Add(new Vertex2D(player.Center + v4 + normalDir * widti * Nar, color, new Vector3((float)Math.Sqrt(factor), 1, w)));
                    ...
                    {
                        int num90 = Dust.NewDust(player.Center + v4 + normalDir * -widti * Main.rand.NextFloat(Main.rand.NextFloat(-0.5f, 0.8f), 0.8f) * player.direction, 0, 0, ModContent.DustType<MiscDusts.Crow>(), 0f, 0f, 0, default(Color), Main.rand.NextFloat(0.4f, 2.5f));
            ...
            else
            {
                for (int i = 1; i < Projectile.oldPos.Length; ++i)
                {
                    ...
                    bart.Add(new Vertex2D(player.Center + v4 + normalDir * widti * Nar, color, new Vector3((float)Math.Sqrt(factor + h), 1, w)));
            ...
            if (ExtraKnife)
            {
                for (int i = 1; i < (Projectile.timeLeft - 25) * 2; ++i)
                {
                    ...
                    bars.Add(new Vertex2D(player.Center + v4 + normalDir * width * Nar, color, new Vector3((float)Math.Sqrt(factor), 1, w)));
                    bars.Add(new Vertex2D(player.Center + v4 + normalDir * -width * dNar, color, new Vector3((float)Math.Sqrt(factor), 0, w)));
                }
            }
            else
            {
                for (int i = 1; i < Projectile.oldPos.Length; ++i)
                {
                    ...
                    bars.Add(new Vertex2D(player.Center + v4 + normalDir * width * Nar, color, new Vector3((float)Math.Sqrt(factor + h), 1, w)));
                    bars.Add(new Vertex2D(player.Center + v4 + normalDir * -width * dNar, color, new Vector3((float)Math.Sqrt(factor + h), 0, w)));
                }
            }
