...
namespace Everglow.Sources.Modules.MythModule.MiscProjectiles.Weapon.Summon
{
    public class XmasWhip : ModProjectile
    {
        ...
        private void AI_165_Whip()
        {
            ...
            if (Utils.GetLerpValue(0.1f, 0.7f, t, true) * Utils.GetLerpValue(0.9f, 0.7f, t, true) > 0.5f)
            {
                ...
                Dust dust = Dust.NewDustDirect(rectangle.TopLeft(), rectangle.Width, rectangle.Height, DustID.GreenMoss, 0f, 0f, 100, Color.White, 1f);
        ...
        private void DrawWhip(Projectile proj)
        {
            ...
            //Texture2D value = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/AshBone2").Value;
                    ...
                    if (!Hitj[j])
                    {
                        if ((Main.npc[j].Center - value2).Length() < 60 && !Main.npc[j].dontTakeDamage && !Main.npc[j].friendly && Main.npc[j].active && Collision.CanHit(value2, 1, 1, Main.npc[j].Center, 1, 1))
                        {
                            ...
                            for (int af = 0; af < 35; af++)
                            {
                                ...
                                Dust d = Dust.NewDustDirect(Main.npc[j].Center - new Vector2(2, 2), 0, 0, ModContent.DustType<MiscDusts.PinePin>(), v0.X, v0.Y, Alpha: 0, Scale: Main.rand.NextFloat(0.3f, 1.1f));
                                d.noGravity = true;
                            }
                            for (int z = 0; z < 30; z++)
                            {
                                float num7 = (float)(Main.rand.Next(0, 2000) / 1000f * Math.PI);
                                if (Main.rand.Next(2) == 0 && PreC < 8)
                                {
                                    //SoundEngine.PlaySound(SoundID.Item33, Projectile.Center);
                                    Vector2 v = new Vector2(0, Main.rand.NextFloat(6f, 8f)).RotatedBy(Math.PI * z / 6d + num7);
                                    int num2 = Projectile.NewProjectile(Projectile.InheritSource(Projectile), Main.npc[j].Center + v * 7, v, 336, Projectile.damage, 0.2f, Main.LocalPlayer.whoAmI, player.GetCritChance(DamageClass.Summon), (int)(Projectile.damage * 0.3));
                                    PreC += 1;
                                }
                            }
                        }
            ...
            value = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhip2").Value;
            originalColor = Microsoft.Xna.Framework.Color.OrangeRed;
            ...
            for (int i = 0; i < list.Count - 1; i++)
            {
                ...
                if (i == list.Count - 2)
                {
                    value = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhip3").Value;
                    Main.spriteBatch.Draw(value, value2 - Main.screenPosition, null, color, rotation, origin, scale, SpriteEffects.None, 0f);
                    color = new Color(255, 255, 255, 0);
                }
                if (i % 7 == 1)
                {
                    value = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhipG").Value;
                }
                if (i % 7 == 2)
                {
                    value = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhipR").Value;
                }
                if (i % 7 == 4)
                {
                    value = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhipB").Value;
                }
                if (i % 7 == 6)
                {
                    value = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhipY").Value;
                }
                Main.spriteBatch.Draw(value, value2 - Main.screenPosition, null, color, rotation, origin, scale, SpriteEffects.None, 0f);
                if (Main.rand.Next(34) <= i * vector2.Length() / 20f)
                {
                    Vector2 v0 = new Vector2(0, Main.rand.NextFloat(0.2f, 0.3f)).RotatedByRandom(MathHelper.TwoPi);
                    Dust d = Dust.NewDustDirect(value2 - new Vector2(2, 2), 0, 0, ModContent.DustType<MiscDusts.PinePin>(), v0.X, v0.Y, Alpha: 0, Scale: Main.rand.NextFloat(0.3f, 1.1f));
                    d.noGravity = true;
                }
                value2 += vector2;
                if (i % 7 == 1)
                {
                    ...
                    RoL[i] = rotation;
                    Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhipGGlow").Value, value2 - Main.screenPosition, null, new Color(255, 255, 255, 0), rotation, origin, scale, SpriteEffects.None, 0f);
                }
                if (i % 7 == 2)
                {
                    Lig[i] = value2 - Main.screenPosition;
                    RoL[i] = rotation;
                    Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhipRGlow").Value, value2 - Main.screenPosition, null, new Color(255, 255, 255, 0), rotation, origin, scale, SpriteEffects.None, 0f);
                }
                if (i % 7 == 4)
                {
                    Lig[i] = value2 - Main.screenPosition;
                    RoL[i] = rotation;
                    Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhipBGlow").Value, value2 - Main.screenPosition, null, new Color(255, 255, 255, 0), rotation, origin, scale, SpriteEffects.None, 0f);
                }
                if (i % 7 == 6)
                {
                    Lig[i] = value2 - Main.screenPosition;
                    RoL[i] = rotation;
                    Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhipYGlow").Value, value2 - Main.screenPosition, null, new Color(255, 255, 255, 0), rotation, origin, scale, SpriteEffects.None, 0f);
                }
            }
            for (int i = 0; i < list.Count - 1; i++)
            {
                if (Lig[i] != Vector2.Zero)
                {
                    if (i % 7 == 1)
                    {
                        Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhipGGlow").Value, Lig[i], null, new Color(255, 255, 255, 0), RoL[i], origin, 1, SpriteEffects.None, 0f);
                    }
                    if (i % 7 == 2)
                    {
                        Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhipRGlow").Value, Lig[i], null, new Color(255, 255, 255, 0), RoL[i], origin, 1, SpriteEffects.None, 0f);
                    }
                    if (i % 7 == 4)
                    {
                        Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhipBGlow").Value, Lig[i], null, new Color(255, 255, 255, 0), RoL[i], origin, 1, SpriteEffects.None, 0f);
                    }
                    if (i % 7 == 6)
                    {
                        Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/XmasWhipYGlow").Value, Lig[i], null, new Color(255, 255, 255, 0), RoL[i], origin, 1, SpriteEffects.None, 0f);
                    }
                }
            }
        }
        Vector2[] Lig = new Vector2[50];
