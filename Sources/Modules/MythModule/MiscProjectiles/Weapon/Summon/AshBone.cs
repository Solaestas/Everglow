...
namespace Everglow.Sources.Modules.MythModule.MiscProjectiles.Weapon.Summon
{
    public class AshBone : ModProjectile
    {
        ...
        private void DrawWhip(Projectile proj)
        {
            ...
            value = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/AshBone3").Value;
            originalColor = Microsoft.Xna.Framework.Color.OrangeRed;
            ...
            for (int i = 0; i < list.Count - 1; i++)
            {
                if (i == list.Count - 2)
                {
                    value = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Summon/AshBone4").Value;
                }
                Vector2 vector = list[i];
                Vector2 vector2 = list[i + 1] - vector;
                float rotation = vector2.ToRotation() - 1.5707964f;
