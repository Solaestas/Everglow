    ...
    public class GreenGemBead : ModProjectile
    {
        ...
        public override void AI()
        {
            if (Tokill < 0)
            {
                for (float v = 0; v < Projectile.velocity.Length(); v += 1f)
                {
                    ...
                    {
                        int r2 = Dust.NewDust(Projectile.Center - Vector2.Normalize(Projectile.velocity) * v - new Vector2(4), 0, 0, ModContent.DustType<MiscDusts.GreenEffect2>(), 0, 0, 200, new Color(DrawC / 3f, DrawC / 3f, DrawC / 3f, 0), DrawC + 0.4f);
        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
            ...
            for (int j = 0; j < 18; j++)
            {
                ...
                int ds = Projectile.NewProjectile(null, Projectile.Center + v * 3f + Projectile.velocity, v, ModContent.ProjectileType<MiscProjectiles.Weapon.Legendary.BrokenGem>(), Projectile.damage / 4, 1, Main.myPlayer, 4);
                Main.projectile[ds].scale = DrawC;
            }
            for (int j = 0; j < 30; j++)
            {
                Vector2 v0 = new Vector2(0, Main.rand.NextFloat(2f, 6f)).RotatedByRandom(Math.PI * 2);
                int num22 = Dust.NewDust(Projectile.Center - new Vector2(4, 4) + new Vector2(0, Main.rand.NextFloat(0, 8f)).RotatedByRandom(Math.PI * 2) + Projectile.velocity, 2, 2, ModContent.DustType<MiscDusts.GreenEffect2>(), v0.X, v0.Y, 0, default(Color), DrawC + 0.5f);
            }
