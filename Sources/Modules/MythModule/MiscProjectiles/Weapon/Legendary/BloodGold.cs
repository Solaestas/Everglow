...
namespace Everglow.Sources.Modules.MythModule.MiscProjectiles.Weapon.Legendary
{
    class BloodGold : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            for (int y = 0; y < 17; y++)
            {
                int num90 = Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y) - new Vector2(4, 4) + Projectile.velocity / Projectile.velocity.Length() * 25, 4, 4, ModContent.DustType<MiscDusts.GoldShine>(), 0f, 0f, 100, default(Color), Main.rand.NextFloat(0.3f, 1.4f));
        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
            for (int y = 0; y < 17; y++)
            {
                int num90 = Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y) - new Vector2(4, 4) + Projectile.velocity / Projectile.velocity.Length() * 25, 4, 4, ModContent.DustType<MiscDusts.GoldShine>(), 0f, 0f, 100, default(Color), Main.rand.NextFloat(0.3f, 1.6f));
                Main.dust[num90].noGravity = true;
                Main.dust[num90].velocity = new Vector2(Main.rand.NextFloat(2.0f, 2.5f), Main.rand.NextFloat(1.8f, 2.5f)).RotatedByRandom(Math.PI * 2d);
            }
            Projectile.Kill();
