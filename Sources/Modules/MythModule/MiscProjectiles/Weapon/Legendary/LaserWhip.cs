                    ...
                    if (!Hitj[j])
                    {
                        if ((Main.npc[j].Center - value2).Length() < 60 && !Main.npc[j].dontTakeDamage && !Main.npc[j].friendly && Main.npc[j].active && Collision.CanHit(value2, 1, 1, Main.npc[j].Center, 1, 1))
                        {
                            ...
                            for (int z = 0; z < 12; z++)
                            {
                                ...
                                if (Main.rand.Next(7) == 0 && PreC < 3)
                                {
                                    SoundEngine.PlaySound(SoundID.Item33, Projectile.Center);
                                    Vector2 v = new Vector2(0, 50).RotatedBy(Math.PI * z / 6d + num7);
                                    int num2 = Projectile.NewProjectile(Projectile.InheritSource(Projectile), Main.npc[j].Center, v, ModContent.ProjectileType<MiscProjectiles.Weapon.Legendary.LaserWorm>(), Projectile.damage, 0.2f, Main.LocalPlayer.whoAmI, player.GetCritChance(DamageClass.Summon), Projectile.damage);
                                    Main.projectile[num2].timeLeft = 90;
                                    //Main.projectile[num2].owner = Main.LocalPlayer.whoAmI;
                                    player.AddBuff(ModContent.BuffType<MiscBuffs.LaserWormBuff>(), 90);
                                    PreC += 1;
                                }
                            }
