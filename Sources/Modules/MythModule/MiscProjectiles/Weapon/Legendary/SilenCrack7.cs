...
{
    public class SilenCrack7 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                if (!Filters.Scene["SLECR7"].IsActive())
                {
                    Filters.Scene.Activate("SLECR7");
                }
            }
            if (Projectile.timeLeft < 25)
            {
                if (Filters.Scene["SLECR7"].IsActive())
                {
                    Filters.Scene.Deactivate("SLECR7");
                    ef2.Parameters["Strds"].SetValue(0);
                    ef2.Parameters["DMax"].SetValue(0.001f);
                    Projectile.Kill();
                }
            }
            ef2 = (Effect)ModContent.Request<Effect>("Everglow/Sources/Modules/MythModule/Effects/SilenceCrack7").Value;
            ...
            for (int j = 0; j < 200; j++)
            {
                if ((Main.npc[j].Center - Projectile.Center).Length() < 80 && !Main.npc[j].dontTakeDamage && !Main.npc[j].friendly && Main.npc[j].active)
                {
                    ...
                    int k = Projectile.NewProjectile(Projectile.InheritSource(Projectile), Projectile.Center.X, Projectile.Center.Y, 0, 0, 612, Projectile.damage, Projectile.knockBack, player.whoAmI, 0f, 0f);
                    Main.projectile[k].timeLeft = 30;
                    Main.npc[j].AddBuff(189, 400, false);
                }
            }
