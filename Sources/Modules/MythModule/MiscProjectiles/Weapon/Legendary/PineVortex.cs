    ...
    public class PineVortex : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.velocity.Length() > 10)
            {
                for (int h = 0; h < Projectile.velocity.Length(); h += 5)
                {
                    ...
                    int r = Dust.NewDust(Projectile.Center + h * v0 - new Vector2(4, 4) + Projectile.velocity / Projectile.velocity.Length() * 12f, 0, 0, ModContent.DustType<MiscDusts.Freeze>(), vz.X, vz.Y, 0, default(Color), St / 700f);
            ...
            else
            {
                ...
                Main.dust[r1].noGravity = true;
                vz = new Vector2(0, Main.rand.NextFloat(0.1f, 1.2f)).RotatedByRandom(MathHelper.TwoPi);
                int r = Dust.NewDust(Projectile.Center - new Vector2(4, 4) + Projectile.velocity / Projectile.velocity.Length() * 12f, 0, 0, ModContent.DustType<MiscDusts.Freeze>(), vz.X, vz.Y, 0, default(Color), St / 700f);
                Main.dust[r].noGravity = true;
            }
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int i = 0; i < 40; i++)
            {
                ...
                int num5 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, ModContent.DustType<MiscDusts.Freeze>(), 0f, 0f, 100, Color.White, (float)(4f * Math.Log10(Projectile.damage)) * St / 4000f);
            ...
            for (int i = 0; i < 30; i++)
            {
                ...
                int num5 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y) + v * 20f, Projectile.width, Projectile.height, ModContent.DustType<MiscDusts.Freeze>(), 0f, 0f, 100, Color.White, (float)(12f * Math.Log10(Projectile.damage)) * St / 4000f);
