            ...
            if (!Main.gamePaused)
            {
                ...
                for (int i = 0; i < 256; i++)
                {
                    if (inAdd == 1 && Main.rand.Next(3) == 0)
                    {
                        ...
                        int G = (int)(((Projectile.ai[0] % 1) * 1000f + 9) * Strengt / 3f);
                        int B = (int)((Projectile.ai[1] - 2) * Strengt / 3f);
                        int num90 = Dust.NewDust(new Vector2(Main.player[i].Center.X, Main.player[i].Center.Y) - new Vector2(37.5f, 27.5f), 60, 80, ModContent.DustType<MiscDusts.Buff>(), 0f, 0f, 0, new Color(R, G, B, 0), Main.rand.NextFloat(0.8f, 1.2f));
