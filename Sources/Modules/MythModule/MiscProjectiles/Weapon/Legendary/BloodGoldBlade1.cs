...
namespace Everglow.Sources.Modules.MythModule.MiscProjectiles.Weapon.Legendary
{
    class BloodGoldBlade1 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                Projectile.NewProjectile(Projectile.InheritSource(Projectile), player.Center, (v1 + new Vector2(29, 29)) / 2f, ModContent.ProjectileType<MiscProjectiles.Weapon.Legendary.BloodGold>(), Projectile.damage / 3 * 2, Projectile.knockBack, player.whoAmI, player.GetCritChance(DamageClass.Melee) + 8, 0f);
            }
            if (Projectile.timeLeft % 190 == 45)
            {
                Projectile.NewProjectile(Projectile.InheritSource(Projectile), player.Center + new Vector2(0, Main.rand.NextFloat(190f, 245f)).RotatedByRandom(Math.PI * 2d), Vector2.Zero, ModContent.ProjectileType<MiscProjectiles.Weapon.Legendary.BloodDarkSmog>(), Projectile.damage * 2, Projectile.knockBack, player.whoAmI, 0f, 0f);
            }
        }
