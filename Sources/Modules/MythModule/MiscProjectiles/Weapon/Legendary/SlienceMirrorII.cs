            ...
            if (ExtraKnife)
            {
                for (int i = 1; i < Maxz; ++i)
                {
                    ...
                    if (!Main.gamePaused)
                    {
                        ...
                        {
                            Projectile.NewProjectile(Projectile.InheritSource(Projectile), player.Center + v4 + normalDir, normalDir.RotatedBy(1.57) * -15f, ModContent.ProjectileType<MiscProjectiles.Weapon.Legendary.SlienceMirrorIIBroken>(), Projectile.damage, 1, Main.myPlayer, Projectile.ai[0], 0f);
                        }
                    }
                }
            }
