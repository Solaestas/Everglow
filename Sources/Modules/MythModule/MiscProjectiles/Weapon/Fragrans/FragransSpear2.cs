...
{
	class FragransSpear2 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            v0.X *= Pdir;
            Vector2 v1 = new Vector2(v0.X, v0.Y * ka).RotatedBy(Prot) * 2f - new Vector2(60, 60);
        ...
        {
            Player player = Main.player[Projectile.owner];
            Texture2D t = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/MiscProjectiles/Weapon/Fragrans/FragransSpear").Value;
            ...
            if (ExtraKnife)
            {
                for (int k = 0; k < (Projectile.timeLeft - 25) * 2; k++)
                {
                    if (k % 10 == 2)
                    {
                        ...
                        Vector2 v3 = v_1.RotatedBy(1.6 / 170d * Math.PI * (Projectile.timeLeft * K0 - 240 + k * K0)) * 1.5f;
                        if (Projectile.timeLeft < 30)
                        {
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            if (ExtraKnife)
            {
                ...
                {
                    float K0 = (Projectile.timeLeft - 30) / 40f;
                    Vector2 v3 = v_1.RotatedBy(1.6 / 170d * Math.PI * (Projectile.timeLeft * K0 - 240 + i)) * 1.5f;
                    if (Projectile.timeLeft < 30)
                    {
                        v3 = v_1.RotatedBy(1.6 * Math.PI) * 1.5f;
                    }
