            ...
            for (int x = X0; x < 58; x++)
            {
                if (player.inventory[x].accessory)
                {
                    ...
                    for (int h = 0; h < 20; h++)
                    {
                        ...
                        int r = Dust.NewDust(new Vector2(player.Center.X, player.Center.Y) - new Vector2(4, 4), 0, 0, ModContent.DustType<TheTusk.Dusts.PurePurple>(), 0, 0, 0, default(Color), 15f * Main.rand.NextFloat(0.4f, 1.1f));
                        Main.dust[r].noGravity = true;
                        Main.dust[r].velocity = v3;
