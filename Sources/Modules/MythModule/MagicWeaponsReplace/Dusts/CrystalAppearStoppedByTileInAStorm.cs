namespace Everglow.Sources.Modules.MythModule.MagicWeaponsReplace.Dusts
{
    public class CrystalAppearStoppedByTileInAStorm : ModDust
    {
        ...
        public override bool Update(Dust dust)
        {
            ...
            {
                dust.velocity = dust.velocity * 0.75f + new Vector2(Utils.SafeNormalize(((AimCenter + new Vector2(xCoefficient * (float)(Math.Sin(Main.timeForVisualEffects * 0.1 + dust.color.G)),0)) - dust.position), new Vector2(0, 0.05f)).X, -dust.color.A/*颜色透明度用来存上升系数*/ / 700f) * 0.25f / (float)(dust.color.B) * 500f;
            }
