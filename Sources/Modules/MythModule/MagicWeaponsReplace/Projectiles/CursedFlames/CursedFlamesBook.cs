...
{
    internal class CursedFlamesBook : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (int i = 0; i < 10; ++i)
            {
                ...
                rot += Projectile.rotation;
                Vector2 BasePos = Projectile.Center + X0 - X0.RotatedBy(rot) * i / 4.5f;
                Dust d0 = Dust.NewDustDirect(BasePos - Y0, 0, 0, DustID.CursedTorch);
                d0.noGravity = true;
                Dust d1 = Dust.NewDustDirect(BasePos + Y0, 0, 0, DustID.CursedTorch);
                d1.noGravity = true;
            }
            for (int i = 0; i < 14; ++i)
            {
                ...
                Dust d0 = Dust.NewDustDirect(BasePos - X0, 0, 0, DustID.CursedTorch);
                d0.noGravity = true;
                Dust d1 = Dust.NewDustDirect(BasePos + X0, 0, 0, DustID.CursedTorch);
                d1.noGravity = true;
            }
        }
    }
