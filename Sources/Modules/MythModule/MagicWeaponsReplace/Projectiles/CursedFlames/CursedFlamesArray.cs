    ...
    internal class CursedFlamesArray : ModProjectile, IWarpProjectile
    {
        ...
        private static void DrawTexCircle(float radious, float width, Color color, Vector2 center, Texture2D tex, double addRot = 0)
        {
            ...
            for (int h = 0; h < radious / 2; h++)
            {
                ...
                circle.Add(new Vertex2D(center + new Vector2(0, radious + width + (float)(Math.Sin(Main.timeForVisualEffects / 4 + h / 2d) * 6f)).RotatedBy(h / radious * Math.PI * 4 + addRot), color, new Vector3(h * 2 / radious, 0, 0)));
            }
        ...
        public void DrawWarp()
        {
            ...
            Player player = Main.player[Projectile.owner];
            DrawTexCircle(Timer * 1.2f, 52, new Color(64, 70, 255, 0), player.Center + RingPos - Main.screenPosition, MythContent.QuickTexture("MagicWeaponsReplace/Projectiles/FogTrace"), Main.timeForVisualEffects / 17);
            Main.spriteBatch.End();
