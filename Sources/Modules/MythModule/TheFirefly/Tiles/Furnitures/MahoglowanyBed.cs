...
namespace Everglow.Sources.Modules.MythModule.TheFirefly.Tiles.Furnitures
{
	public class MahoglowanyBed : ModTile
	{
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY) {
			Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 64, 32, ModContent.ItemType<Items.Furnitures.MahoglowanyBed>());
		}
		...
		public override void MouseOver(int i, int j) {
			...
			if (!Player.IsHoveringOverABottomSideOfABed(i, j)) {
				...
				player.noThrow = 2;
				player.cursorItemIconEnabled = true;
				player.cursorItemIconID = ModContent.ItemType<Items.Furnitures.MahoglowanyBed>();
			}
		}
	}
}
