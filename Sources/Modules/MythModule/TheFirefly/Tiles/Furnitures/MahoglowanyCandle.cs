...
{
	public class MahoglowanyCandle : ModTile
	{
		...
		public override void SetStaticDefaults() {
			...
			name.SetDefault("Mahoglowany Candle");
			AddMapEntry(new Color(0, 14, 175), name);
			if (!Main.dedServ)

				if (!Main.dedServ)
			{
				flameTexture = ModContent.Request<Texture2D>("Everglow/Sources/Modules/MythModule/TheFirefly/Tiles/Furnitures/MahoglowanyCandle_Flame");
			}
		}
		...
		{
			Item.NewItem(new EntitySource_TileBreak(x, y), x * 16, y * 16, 48, 32, ModContent.ItemType<Items.Furnitures.MahoglowanyCandle>());
		}
	}
}
