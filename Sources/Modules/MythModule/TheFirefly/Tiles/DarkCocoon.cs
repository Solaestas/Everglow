namespace Everglow.Sources.Modules.MythModule.TheFirefly.Tiles
{
    public class DarkCocoon : ModTile
    {
        ...
        public override void RandomUpdate(int i, int j)
        {
            ...
            {
                if (!Main.tile[i, j].IsHalfBlock && !Main.tile[i + 1, j].IsHalfBlock && !Main.tile[i - 1, j].IsHalfBlock && !Main.tile[i + 2, j].IsHalfBlock && !Main.tile[i - 2, j].IsHalfBlock)//树木
                {
                    for (int x = -2; x < 3; x++)
                    {
                        for (int y = -2; y < 0; y++)
            ...
            if (!Main.tile[i, j - 1].HasTile && !Main.tile[i + 1, j - 1].HasTile && !Main.tile[i - 1, j - 1].HasTile && !Main.tile[i, j].IsHalfBlock && !Main.tile[i - 1, j].IsHalfBlock && !Main.tile[i + 1, j].IsHalfBlock)//黑萤苣
            {
