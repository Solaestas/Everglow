...
internal class PlayerColliding : ModPlayer
{
    ...
    internal static void Player_DryCollision(On.Terraria.Player.orig_DryCollision orig, Player self, bool fallThrough, bool ignorePlats)
    {
    ...
    internal static void Player_WaterCollision(On.Terraria.Player.orig_WaterCollision orig, Player self, bool fallThrough, bool ignorePlats)
    {
        if (TileSystem.Enable)
        {
            var player = self.GetModPlayer<PlayerColliding>();
            ...
            //Debug.Assert(self.velocity.Y == 0);
            TileSystem.EnableDTCollision = true;
            return;
        }
        orig(self, fallThrough, ignorePlats);
    ...
    internal static void Player_HoneyCollision(On.Terraria.Player.orig_HoneyCollision orig, Player self, bool fallThrough, bool ignorePlats)
    {
        if (TileSystem.Enable)
        {
            var player = self.GetModPlayer<PlayerColliding>();
    ...
    internal static bool Player_CanFitSpace(On.Terraria.Player.orig_CanFitSpace orig, Player self, int heightBoost)
    {
        TileSystem.EnableDTCollision = false;
    ...
    internal static void Player_WallslideMovement(ILContext il)
    {
        ...
        cursor.GotoNext(MoveType.Before, ins => ins.OpCode == OpCodes.Ret);
        cursor.Emit(OpCodes.Ldarg_0);
        cursor.EmitDelegate((Player player) =>
        {
    ...
    internal static void Player_ItemCheck_UseMiningTools_ActuallyUseMiningTool(On.Terraria.Player.orig_ItemCheck_UseMiningTools_ActuallyUseMiningTool orig, Player self, Item sItem, out bool canHitWalls, int x, int y)
    {
        //TODO 大概联机会炸……
