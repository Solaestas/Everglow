...
internal abstract class Invasion : IModule
{
    ...
    {
        Main.invasionProgressMax = progressMax;//最大进度
        Main.invasionProgress = progress;//进度
        Main.invasionProgressMode = 2;//是否到达
        Main.invasionType = InvasionID;//种类
        Main.invasionProgressNearInvasion = true;//不知道
        Main.invasionProgressDisplayLeft = 160;//淡出的计时器
        Main.invasionProgressAlpha = 1;//淡出的透明度
        Main.invasionProgressWave = wave;//波次
        Main.invasionSize = 1;//不知道
        Main.invasionSizeStart = 1;//不知道
    }
    public virtual void DrawBar()
    {
        ...
        float waveRate = Main.invasionProgressMax == 0 ?
            1 :
