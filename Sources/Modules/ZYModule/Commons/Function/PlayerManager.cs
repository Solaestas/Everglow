...
internal class PlayerManager : ModPlayer
{
    ...
    public override void PostUpdate()
    {
        ...
        {
            foreach (var player in Main.player.Where(p => p.active).Select(p => p.GetModPlayer<PlayerManager>()))
            {
                Console.WriteLine($"{player.Name}, Left:{player.MouseLeft.Press}, Right:{player.MouseRight.Press}, Position:{player.MouseWorld}");
            }
        }
    }
