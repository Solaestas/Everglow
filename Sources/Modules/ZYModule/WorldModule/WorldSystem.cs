...
internal class WorldSystem : IModule
{
    ...
    {
        if (dataToWorld.ContainsKey(data) && HookException.Check("WorldIcon_ctor_Error"))
        {
            try
            {
                dataToUI[data] = self;
                ...
                while (type != null)
                {
                    Fields = Fields.Concat(type.GetFields(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance));
                    Methods = Methods.Concat(type.GetMethods(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance));
                    type = type.BaseType;
                }
                ...
                {
                    if (!fName.ContainsKey(field.Name))
                    {
                ...
                {
                    if (!mName.ContainsKey(method.Name))
                    {
                ...
                mName["LoadTextures"].Invoke(self, null);
                mName["InitializeAppearance"].Invoke(self, null);
            ...
            {
                HookException.Silent("WorldIcon_ctor_Error", ex);
