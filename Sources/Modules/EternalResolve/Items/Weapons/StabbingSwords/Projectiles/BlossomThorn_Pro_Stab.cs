...
namespace Everglow.EternalResolve.Items.Weapons.StabbingSwords.Projectiles
{
	public class BlossomThorn_Pro_Stab : StabbingProjectile_Stab
	{
		...
		public override void DrawEffect(Color lightColor)
		{
			...
			normalized = Vector2.Normalize(Projectile.velocity.RotatedBy(Math.PI * 0.5)) * Math.Min(72 * ToKill / 20f, 72) * StabEffectWidth;
			...
			normalized = Vector2.Normalize(Projectile.velocity.RotatedBy(Math.PI * 0.5)) * 36 * ToKill / 120f * StabEffectWidth;
			bars = new List<Vertex2D>
			{
				new Vertex2D(start + normalized, new Color(0, 0, 0, 0), new Vector3(1 + time, 0, 0)),
				new Vertex2D(start - normalized, new Color(0, 0, 0, 0), new Vector3(1 + time, 1, 0)),
