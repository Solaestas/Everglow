...
namespace Everglow.EternalResolve.Projectiles
{
	public class BlossomThorn_Pro_Stab : StabbingProjectile_Stab
	{
		...
		{
			StabColor = new Color(209, 187, 107);
			StabShade = 0.2f;
			StabDistance = 0.70f;
		...
		public override void DrawEffect(Color lightColor)
		{
			...
			normalized = Vector2.Normalize(Projectile.velocity.RotatedBy(Math.PI * 0.5)) * Math.Min(72 * StabTimer / 20f, 72) * StabEffectWidth;
			...
			normalized = Vector2.Normalize(Projectile.velocity.RotatedBy(Math.PI * 0.5)) * 36 * StabTimer / 120f * StabEffectWidth;
			bars = new List<Vertex2D>
			{
				new Vertex2D(start + normalized, new Color(0, 0, 0, 0), new Vector3(1 + time, 0, 0)),
				new Vertex2D(start - normalized, new Color(0, 0, 0, 0), new Vector3(1 + time, 1, 0)),
		...
		public override void AI()
		{
			if (Main.rand.NextBool(12))
			{
				...
				{
					var dust = Dust.NewDustDirect(pos, 0, 0, ModContent.DustType<LeafDust>(), 0, 0, 0, default, Main.rand.NextFloat(0.95f, 1.7f));
					dust.velocity = vel;
				}
			}
			if (Main.rand.NextBool(12))
			{
				...
				if (Collision.CanHit(Projectile.Center - Projectile.velocity, 0, 0, pos + vel, 0, 0))
				{
					var dust = Dust.NewDustDirect(pos, 0, 0, ModContent.DustType<PlumBlossom>(), 0, 0, 0, default, Main.rand.NextFloat(0.95f, 1.7f));
					dust.velocity = vel;
				}
			}
			base.AI();
