...
public class RedGlassbulbBand_bulb : CableTile
{
	...
	public override void DrawCable(Rope rope, Point pos, SpriteBatch spriteBatch, TileDrawing tileDrawing, Color color = default)
	{
		...
		Texture2D tex = PaintedTextureSystem.TryGetPaintedTexture(ModAsset.RedGlassbulbBand_bulb_Path, type, 1, paint, tileDrawing);
		tex ??= ModAsset.RedGlassbulbBand_bulb.Value;
			...
			if (thisMass.Value == 0.2f)
			{
				switch (style)
				{
					...
					case 1:
						...
						Lighting.AddLight(thisMass.Position, new Vector3(0.5f, 0f, 0f));
					...
					case 2:
						...
						Lighting.AddLight(thisMass.Position, new Vector3(0.5f, 0f, 0f) * light); // blue light is dimmer than red and green, so it's 0.7f.
						...
						break;
					case 3:
						timeValue = (float)Main.time * 0.02f;
						light = (MathF.Sin(timeValue) + 1) * 0.6f;
						if (light > 0)
						{
							spriteBatch.Draw(tex, drawPos, new Rectangle(2, 6, 6, 18), tileLight, rotation, new Vector2(3f, 0), 0.7f, tileSpriteEffect, 0);
						}
