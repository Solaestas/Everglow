...
public class Georg_LightingBoltWave : Visual
{
	...
	public override void Draw()
	{
		...
		for (int i = 0; i <= sideCount; ++i)
		{
			...
			var drawC = new Color(0.7f, 0.7f, 0.7f, 0.7f);
			...
			Vector2 innerStar = star.SafeNormalize(Vector2.zeroVector) * width;
		...
		{
			Vector2 drawPos = position;
			var drawC = new Color(0.1f, 0.7f, 1f, 0);
			if (timer < 12 && timer > 4)
			{
				if (timer % 8 < 4)
				{
					drawC = Color.Lerp(new Color(0.5f, 0.8f, 1f, 0), new Color(0.3f, 0f, 0.4f, 0), i % 8 * 0.25f);
				}
			}
			if (timer <= 4)
			{
