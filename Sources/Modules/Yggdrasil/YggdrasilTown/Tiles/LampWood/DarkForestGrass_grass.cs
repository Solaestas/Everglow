...
public class DarkForestGrass_grass_fore : ForegroundVFX
{
	...
	public void AddDrawingFace(List<Vertex2D> bars, Vector2 direction, Vector2 position, Color lightColor, float length = 1, Vector2 offset = new Vector2())
	{
		...
		bars.Add(drawCenter + new Vector2(8 * length, 8).RotatedBy(rot) + direction * 8, lightColor, new Vector3(dLength + 18 * style / 52f, 1, 0));

		bars.Add(drawCenter + new Vector2(-8 * length, 8).RotatedBy(rot) + direction * 8, lightColor, new Vector3(18 * style / 52f, 1, 0));
		bars.Add(drawCenter + new Vector2(8 * length, 8).RotatedBy(rot) + direction * 8, lightColor, new Vector3(dLength + 18 * style / 52f, 1, 0));
		bars.Add(drawCenter + new Vector2(-8 * length, -8).RotatedBy(rot) + direction * 8 + windPush, lightColor, new Vector3(18 * style / 52f, 0, 0));//这三个点需要碰撞
	}
}
