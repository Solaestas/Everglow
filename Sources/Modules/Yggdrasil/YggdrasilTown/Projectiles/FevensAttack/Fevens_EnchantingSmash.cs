...
public class Fevens_EnchantingSmash : ModProjectile
{
	...
	public override void AI()
	{
		...
		{
			Projectile p0 = Projectile.NewProjectileDirect(Projectile.GetSource_FromAI(), EndPosition, Vector2.zeroVector, ModContent.ProjectileType<Fevens_EnchantingSmash_Explosion>(), 260, 2, default, Projectile.Center.X, Projectile.Center.Y);
		}
	}
	...
	public override bool PreDraw(ref Color lightColor)
	{
		...
		List<Vertex2D> predictLine = new List<Vertex2D>();
		List<Vertex2D> predictLineDark = new List<Vertex2D>();
		for (int i = 0; i < 120; i++)
		{
		...
		List<Vertex2D> predictRange = new List<Vertex2D>();
		List<Vertex2D> predictRangeDark = new List<Vertex2D>();

		List<Vertex2D> predictDuration = new List<Vertex2D>();
		List<Vertex2D> predictDurationDark = new List<Vertex2D>();
		for (int i = 0; i < 200; i++)
		{
	...
	public void DrawStar(Vector2 position, float scale = 1f)
	{
		...
		Color darkDraw = Color.Lerp(Color.White * 0.3f, Color.White * 0.8f, timeLeftValue);
		Color starColor = Color.Lerp(new Color(0.2f, 0.05f, 0.4f, 0), new Color(0.6f, 0.25f, 1f, 0), timeLeftValue);
		Color starColor2 = Color.Lerp(new Color(0.6f, 0.25f, 1f, 0), new Color(1f, 0.85f, 1f, 0), timeLeftValue);
