...
public class BrittleRockSlingshotStone_Explosion : ModProjectile, IWarpProjectile
{
	...
	public override void AI()
	{
		...
		if (Projectile.timeLeft == 199)
		{
			...
			for (int x = 0; x < 12; x++)
			{
				Dust d = Dust.NewDustDirect(Projectile.position - new Vector2(4), Projectile.width, Projectile.height, DustID.WitherLightning, 0f, 0f, 0, default, Main.rand.NextFloat(0.4f, 0.9f));
	...
	public override bool PreDraw(ref Color lightColor)
	{
		...
		Color c = new Color(3f * (1 - timeValue), 0.6f * (1 - timeValue) * (1 - timeValue) * (1 - timeValue), 1.9f * (1 - timeValue) * (1 - timeValue), 0f);
