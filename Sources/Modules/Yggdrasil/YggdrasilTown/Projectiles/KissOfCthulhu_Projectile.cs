...
public class KissOfCthulhu_Projectile : ModProjectile
{
	...
	public override void AI()
	{
		...
		for (int i = 0; i < Projectile.velocity.Length(); i++)
		{
			...
			Vector4 cus = new Vector4(DustCount, Projectile.velocity.ToRotationSafe(), 0, 0);
			float speed = 2.5f;
			if (cus.X < 90f)
			{
				speed *= cus.X / 90f;
	...
	public override void OnKill(int timeLeft)
	{
		for (int i = 0; i < 15; i++)
		{
			Dust dust = Dust.NewDustDirect(Projectile.Center, 0, 0, DustID.Shadowflame, Projectile.velocity.X, Projectile.velocity.Y);
