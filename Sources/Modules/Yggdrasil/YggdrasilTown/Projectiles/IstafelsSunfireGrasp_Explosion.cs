...
public class IstafelsSunfireGrasp_Explosion : ModProjectile, IWarpProjectile_warpStyle2
{
	...
	public override bool PreDraw(ref Color lightColor)
	{
		...
		List<Vertex2D> bars2_dark = new List<Vertex2D>();
		for (int i = 0; i <= resolutionCircle; i++)
		{
			...
			Color colorInner = new Color(0, 0, 0, 0);
			...
			Color colorOuter = new Color(0, 0, 0, 0);
	...
	public void DrawWarp(VFXBatch spriteBatch)
	{
		...
		for (int i = 0; i <= resolutionCircle; i++)
		{
			...
			Color colorMiddle = new Color(-dirMiddle.X * 0.5f + 0.5f, -dirMiddle.Y * 0.5f + 0.5f, fade * 1.2f * Main.GameViewMatrix.Zoom.X, 0);
