...
public class BloodySwamp_area : ModProjectile, IWarpProjectile_warpStyle2
{
	...
	public override bool PreDraw(ref Color lightColor)
	{
		...
		List<Vertex2D> bars_outside_black = new List<Vertex2D>();
		for (int i = 0; i <= 150; i++)
		{
			var radius = new Vector2(0, 90 * size).RotatedBy(i / 150f * MathHelper.TwoPi);
		...
		List<Vertex2D> bars_outside = new List<Vertex2D>();
		for (int i = 0; i <= 150; i++)
		{
			var radius = new Vector2(0, 90 * size).RotatedBy(i / 150f * MathHelper.TwoPi);
			var radius2 = new Vector2(0, 120 * size).RotatedBy(i / 150f * MathHelper.TwoPi);
	...
	public void DrawWarp(VFXBatch spriteBatch)
	{
		...
		for (int i = 0; i <= 150; i++)
		{
			...
			Color colorMiddle = new Color(-dirMiddle.X * 0.5f + 0.5f, -dirMiddle.Y * 0.5f + 0.5f, fade, 0);
