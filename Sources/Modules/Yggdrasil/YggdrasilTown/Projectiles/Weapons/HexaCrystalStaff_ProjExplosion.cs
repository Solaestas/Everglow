...
public class HexaCrystalStaff_ProjExplosion : ModProjectile, IWarpProjectile_warpStyle2
{
	...
	public override bool PreDraw(ref Color lightColor)
	{
		...
		List<Vertex2D> hexagon = new List<Vertex2D>();
		List<Vertex2D> hexagonSide = new List<Vertex2D>();
		List<Vertex2D> hexagonSideDark = new List<Vertex2D>();
		List<Vertex2D> hexagonSideRainbow = new List<Vertex2D>();
	...
	public void DrawWarp(VFXBatch batch)
	{
		...
		for (int i = 0; i <= 6; i++)
		{
			...
			Color warpColor = new Color(dir.X + 0.5f, dir.Y + 0.5f, Projectile.timeLeft / 120f, 1);
			Color warpColorInner = new Color(dir.X + 0.5f, dir.Y + 0.5f, Projectile.timeLeft / 120f, 1);
