...
public class WaterErodedGreenBrick : ModTile
{
	public override void SetStaticDefaults()
	{
		...
		AddMapEntry(new Color(66, 98, 118));
	}
	...
	public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
	{
		...
		if (!Ins.VisualQuality.High)
		{
			...
			var frame = new Rectangle(i % 12 * 16, 180 + j % 12 * 16, 16, 16);
			spriteBatch.Draw(texture, drawPos, frame, Lighting.GetColor(i, j), 0, frame.Size() * 0.5f, 1, SpriteEffects.None, 0);
			var frameSide = new Rectangle(tile.TileFrameX, tile.TileFrameY + 90, 16, 16);
		...
		else
		{
			...
			for (int x = 0; x < 3; x++)
			{
				for (int y = 0; y < 3; y++)
				{
					var frame = new Rectangle(i % 12 * 16 + x * 6, 180 + j % 12 * 16 + y * 6, 6, 6);
					...
					var drawColor = Color.Lerp(offsetColor, lightColor, 0.5f);
