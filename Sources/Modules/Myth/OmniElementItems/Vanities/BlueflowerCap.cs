...
namespace Everglow.Sources.Modules.MythModule.OmniElementItems.Vanities
{
	...
	public class BlueflowerCap : ModItem
	{
		...
		public override void Load()
		{
			...
			Terraria.On_Main.DrawPlayers_BehindNPCs += DrawDress;
		}

		private static void DrawDress(Terraria.On_Main.orig_DrawPlayers_BehindNPCs orig, Terraria.Main self)
		{
		...
		private static void DrawHat(Terraria.On_Main.orig_DrawPlayers_AfterProjectiles orig, Terraria.Main self)
		{
			orig(self);
			Main.spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.Default, RasterizerState.CullNone, null, Main.GameViewMatrix.TransformationMatrix);
			for (int d = 0; d < Main.player.Length; d++)
