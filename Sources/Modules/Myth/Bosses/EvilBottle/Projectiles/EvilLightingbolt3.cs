...
{
    public class EvilLightingbolt3 : ModProjectile
    {
        ...
        public override void AI()
        {
            if (Projectile.timeLeft == 11)
            {
                ...
                Vector2 v = new Vector2(Projectile.ai[0], Projectile.ai[1]);
                for (int i = 1; i < 5000; i++)
                {
                    vL[i] = vL[i - 1] + (v - vL[i - 1]).RotatedBy(Main.rand.NextFloat(-0.5f, 0.5f)) * Main.rand.NextFloat(2f,17f) / (v - vL[i - 1]).Length();
                    if((vL[i] - v).Length() < 8)
                    {
                        break;
                    }
