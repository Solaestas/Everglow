...
{
    public class DarkFlameMagic2 : ModProjectile
	{
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            for (int i = 1; i <= 4; ++i)
            {
                ...
                bars.Add(new CustomVertexInfo(Projectile.Center + new Vector2(0, r + 40).RotatedBy(i / 3d * Math.PI * 2 + Rot), color, new Vector3((float)Math.Sqrt(factor), 1, w)));
                bars.Add(new CustomVertexInfo(Projectile.Center + new Vector2(0, r).RotatedBy(i / 3d * Math.PI * 2 + Rot), color, new Vector3((float)Math.Sqrt(factor), 0, w)));
            }
