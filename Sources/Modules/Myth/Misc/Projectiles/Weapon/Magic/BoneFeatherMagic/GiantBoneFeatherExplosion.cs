...
public class GiantBoneFeatherExplosion : ModProjectile, IWarpProjectile
{
	...
	public override void PostDraw(Color lightColor)
	{
		...
		Main.spriteBatch.Draw(Shadow, Projectile.Center - Main.screenPosition, null, new Color(0.9f * (1 - timeValue), 0.9f * (1 - timeValue), 0.6f * (1 - timeValue), 0f) * dark, 0, Shadow.Size() / 2f, 2.2f * Projectile.ai[0] / 15f * dark, SpriteEffects.None, 0);
		DrawTexCircle(MathF.Sqrt(timeValue) * 24 * Projectile.ai[0], 8 * (1 - timeValue) * Projectile.ai[0], new Color(0.9f * (1 - timeValue), 0.9f * (1 - timeValue), 0.6f * (1 - timeValue), 0f), Projectile.Center - Main.screenPosition, Commons.ModAsset.Trail_6.Value);
	}
