	...
	public override void Smash(int level = 0)
	{
		...
		if(level == 0)
		{
			for (int x = 0; x < 8; x++)
			{
				...
				Projectile p0 = Projectile.NewProjectileDirect(null, player.Center - v * 3 + v2 + new Vector2(0, -74), v, ModContent.ProjectileType<SpikeClubSlash>(), Projectile.damage / 2, 0, player.whoAmI, Main.rand.NextFloat(-0.05f, 0.05f));
		...
		if (level == 1)
		{
			for (int x = 0; x < 24; x++)
			{
				...
				Projectile p0 = Projectile.NewProjectileDirect(null, player.Center - v * 3 + v2 + new Vector2(0, -134), v, ModContent.ProjectileType<SpikeClubSlash>(), Projectile.damage / 2, 0, player.whoAmI, Main.rand.NextFloat(-0.05f, 0.05f));
