...
public class ChlorophyteClub_smash : ClubProjSmash_Reflect
{
	...
	public override void Smash(int level)
	{
		if (level == 0)
		{
			for (int i = 0; i < 3; i++)
			{
				Vector2 v0 = new Vector2(0, -1 * Owner.gravDir).RotatedBy((i - 1) / 6f * MathHelper.TwoPi);
			...
			Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<ChlorophyteClub_VFX_2>(), Projectile.damage, Projectile.knockBack * 0.4f, Projectile.owner, 0.4f, Owner.gravDir);
		}
		if (level == 1)
		{
			for (int i = 0; i < 5; i++)
			{
				Vector2 v0 = new Vector2(0, -1 * Owner.gravDir).RotatedBy((i - 2) / 10f * MathHelper.TwoPi);
			...
			for (int i = 0; i < 6; i++)
			{
				Vector2 v0 = new Vector2(0, -1 * Owner.gravDir).RotatedBy((i - 2.5f) / 10f * MathHelper.TwoPi);
			...
			Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<ChlorophyteClub_VFX_2>(), Projectile.damage, Projectile.knockBack * 0.4f, Projectile.owner, 0.7f, Owner.gravDir);
		}
		base.Smash(level);
	}
}