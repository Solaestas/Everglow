...
public class OrichalcumClub_smash : ClubProj_Smash_metal
{
	...
	public override void Smash(int level)
	{
		...
		if (level == 1)
		{
			for (int x = 0; x < 17; x++)
			{
				Projectile p0 = Projectile.NewProjectileDirect(null, player.Center + new Vector2(0, -14), Vector2.zeroVector, ModContent.ProjectileType<OrichalcumPedal_slash>(), Projectile.damage / 2, 0, player.whoAmI, 1.85f);
