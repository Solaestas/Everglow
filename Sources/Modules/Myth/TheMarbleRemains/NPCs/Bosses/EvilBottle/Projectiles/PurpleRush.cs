    ...
    public class PurpleRush : ModProjectile
	{
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            if(BoomTime > 0)
            {
                ...
                Main.spriteBatch.Draw(MythContent.QuickTexture("TheMarbleRemains/NPCs/Bosses/EvilBottle/Projectiles/PurpleCircle"), Projectile.Center - Main.screenPosition, null, new Color(0.8f, 0.8f, 0.8f, 0) * B, 0f, new Vector2(26, 26), 1, SpriteEffects.None, 0f);
                int[] C = new int[8];
                for (int G = 0; G < 3; G++)
                {
                    if ((int)T % 8 == G)
                ...
                for (int G = 0; G < Projectile.ai[0]; G++)
                {
                    ...
                    Main.spriteBatch.Draw(MythContent.QuickTexture("TheMarbleRemains/NPCs/Bosses/EvilBottle/Projectiles/PurpleArrow"), Projectile.Center - Main.screenPosition + v, null, new Color(0.8f, 0.8f, 0.8f, 0) * C[G], (float)Math.Atan2(vd.Y, vd.X), new Vector2(26, 26), 1, SpriteEffects.None, 0f);
                }
            }
            return false;
        }
