...
{
    public class EvilLightingbolt : ModProjectile
    {
        ...
        public override void AI()
        {
            if (Projectile.timeLeft == 11)
            {
                ...
                for (int i = 1; i < 99; i++)
                {
                    ...
                    {
                        vL[i] = vL[i - 1] + (vL[i - 1] - vL[0]) / (vL[i - 1] - vL[0]).Length() * Main.rand.NextFloat(3, 17f) + new Vector2(0, Main.rand.NextFloat(0, 10f)).RotatedByRandom(Math.PI * 2);
                    }
                    if(Main.rand.Next(100) < 9)
                    {
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), vL[i].X, vL[i].Y, 0, 0, ModContent.ProjectileType<EvilLightingbolt2>(), 0, 0f, Main.myPlayer, Main.rand.Next(6,48), 0f);
                    }
                }
            }
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            if (bars.Count > 2)
            {
                ...
                var vertex = new CustomVertexInfo((bars[0].Position + bars[1].Position) * 0.5f + (bars[0].Position - bars[1].Position), Color.White, new Vector3(0, 0.5f, 1));
