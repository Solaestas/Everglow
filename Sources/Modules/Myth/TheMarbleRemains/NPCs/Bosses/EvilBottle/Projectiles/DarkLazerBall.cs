    ...
    public class DarkLazerBall : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for(int i = 0;i < 4;i++)
            {
                ...
                if(Sca > 0)
                {
                    Main.spriteBatch.Draw(MythContent.QuickTexture("TheMarbleRemains/NPCs/Bosses/EvilBottle/heatmapColdPurpleCircle"), base.Projectile.Center - Main.screenPosition + new Vector2(0, -r).RotatedBy(Math.PI * n * (i + 1)), null, new Color(1f, 1f, 1f, 0), base.Projectile.rotation, new Vector2(128f, 128f), Sca, SpriteEffects.None, 0f);
                    Main.spriteBatch.Draw(MythContent.QuickTexture("TheMarbleRemains/NPCs/Bosses/EvilBottle/heatmapColdPurpleCircle"), base.Projectile.Center - Main.screenPosition + new Vector2(0, -r).RotatedBy(Math.PI * n * -(i + 1)), null, new Color(1f, 1f, 1f, 0), base.Projectile.rotation, new Vector2(128f, 128f), Sca, SpriteEffects.None, 0f);
                }
                if (Projectile.timeLeft > 32)
                {
                    if (Sca > 0.1f)
                    {
                        for (int k = 0; k < Leng - 52; k += 16)
                        {
                            ...
                            if (k < Leng - 68)
                            {
                                Main.spriteBatch.Draw(MythContent.QuickTexture("TheMarbleRemains/NPCs/Bosses/EvilBottle/Projectiles/EvilLazer"), base.Projectile.Center - Main.screenPosition + new Vector2(0, -r - k - 8).RotatedBy(Math.PI * n * (i + 1)), new Rectangle(0, 0, 16, 16), new Color((Sca - 0.1f) * 10, (Sca - 0.1f) * 10, (Sca - 0.1f) * 10, 0), -(float)(Math.PI * n * -(i + 1) + Math.PI / 2d), new Vector2(8f, 8f), 1, SpriteEffects.None, 0f);
                            }
                            else
                            {
                                Main.spriteBatch.Draw(MythContent.QuickTexture("TheMarbleRemains/NPCs/Bosses/EvilBottle/Projectiles/EvilLazerEnd"), base.Projectile.Center - Main.screenPosition + new Vector2(0, -r - k - 8).RotatedBy(Math.PI * n * (i + 1)), new Rectangle(0, 0, 16, 16), new Color((Sca - 0.1f) * 10, (Sca - 0.1f) * 10, (Sca - 0.1f) * 10, 0), -(float)(Math.PI * n * -(i + 1) + Math.PI / 2d), new Vector2(8f, 8f), 1, SpriteEffects.None, 0f);
                                if (!Main.gamePaused)
                                {
