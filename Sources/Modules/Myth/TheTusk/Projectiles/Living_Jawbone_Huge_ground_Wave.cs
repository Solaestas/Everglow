...
public class Living_Jawbone_Huge_ground_Wave : ModProjectile
{
	...
	public override void AI()
	{
		...
		{
			Projectile projectile = Projectile.NewProjectileDirect(Projectile.GetSource_FromAI(), Projectile.Center, Vector2.zeroVector, ModContent.ProjectileType<Living_Jawbone_Huge_ground>(), Projectile.damage, 4f, Projectile.owner);
		}
	}
	...
	public Vector2 GetWave((Point TileCoord, float Rotation) item, Vector2 offset)
	{
		...
		worldCoord -= offset;
		float wave = MathF.Sin(MathF.Pow(distance, 0.7f) * 0.25f + Projectile.timeLeft * 0.1f) + 1;
		wave *= 15;
		float distanceFade = (400 - distance) / 400f;
		distanceFade = Math.Max(distanceFade, 0);
		distanceFade = MathF.Pow(distanceFade, 0.7f);
		wave *= distanceFade;
		float timeFade = 90 - Math.Abs(Projectile.timeLeft - 90);
