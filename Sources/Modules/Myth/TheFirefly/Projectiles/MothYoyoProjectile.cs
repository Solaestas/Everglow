...
internal class MothYoyoProjectile : ModProjectile
{
	...
	public override bool PreDraw(ref Color lightColor)
	{
		...
		else
		{
			...
			vector.X -= num4 * 0.1f;
			vector.Y -= num5 * 0.1f;
			num4 = Projectile.position.X + Projectile.width * 0.5f - vector.X;
			num5 = Projectile.position.Y + Projectile.height * 0.5f - vector.Y;
		}
		while (flag)
		{
			float num8 = 12f;
			...
			else
			{
				...
				vector.Y += num5;
				num4 = Projectile.position.X + Projectile.width * 0.5f - vector.X;
				num5 = Projectile.position.Y + Projectile.height * 0.1f - vector.Y;
				if (num10 > 12f)
				{
					float num11 = 0.3f;
