...
public class FireflyPiranhaInfected : ModNPC
{
	...
	private void NormalAttack(Player target)
	{
		...
		{
			Vector2 velocityRotation = Vector2.Normalize(toPlayer) * 25f;
		...
		if (toPlayer.Length() > 160f && toPlayer.Length() <= 200f && Collision.CanHit(NPC, target))
		{
			...
			{
				Vector2 velocityRotation = Vector2.Normalize(toPlayer) * 88f;
				NPC.velocity = velocityRotation * 0.55f + NPC.velocity * 0.45f;
			}
		}
		if (MathF.Abs(NPC.velocity.X) < 16f)
		{
		...
		if (MathF.Abs(NPC.velocity.X) > 20f)
		{
		...
		switch (NPCFrameAnimationType)
		{
			...
			case 1:
				if (NPC.frameCounter > 12)
				{
					...
					{
						NPC.frame.Y += frameHeight;
						if(NPC.frame.Y == 432 && NPC.wet)
						{
							NPC.velocity *= 2.8f;
							if(NPC.velocity.Length() > 40f)
							{
								NPC.velocity = NPC.velocity / NPC.velocity.Length() * 40f;
							}
						}
						if (NPC.frame.Y == 648)
						{
	...
	public override void OnKill()
	{
		Gore.NewGore(NPC.GetSource_FromAI(), NPC.Center + new Vector2(0, Main.rand.Next(40)).RotatedByRandom(6.283),
			   new Vector2(0, Main.rand.NextFloat(8)).RotatedByRandom(6.283), ModContent.Find<ModGore>("Everglow/FireflyPiranhaInfected0").Type);
		Gore.NewGore(NPC.GetSource_FromAI(), NPC.Center + new Vector2(0, Main.rand.Next(40)).RotatedByRandom(6.283),
			   new Vector2(0, Main.rand.NextFloat(8)).RotatedByRandom(6.283), ModContent.Find<ModGore>("Everglow/FireflyPiranhaInfected1").Type);
		Gore.NewGore(NPC.GetSource_FromAI(), NPC.Center + new Vector2(0, Main.rand.Next(40)).RotatedByRandom(6.283),
			   new Vector2(0, Main.rand.NextFloat(8)).RotatedByRandom(6.283), ModContent.Find<ModGore>("Everglow/FireflyPiranhaInfected2").Type);
		for (int f = 0; f < 32; f++)
		{
