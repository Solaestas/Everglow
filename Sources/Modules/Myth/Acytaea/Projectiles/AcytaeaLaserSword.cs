...
public class AcytaeaLaserSword : ModProjectile
{
	...
	public void DrawLaser(Texture2D tex, Color color, float width = 36)
	{
		...
		for (int i = 1; i < 600; ++i)
		{
			...
			bars.Add(new Vertex2D(detect + velocityLeft, c0, new Vector3(x, 1, MathF.Min(i / 48f, 0.5f))));
			bars.Add(new Vertex2D(detect - velocityLeft, c0, new Vector3(x, 0, MathF.Min(i / 48f, 0.5f))));
		}
		EndPos = detect;
	...
	public override void PostDraw(Color lightColor)
	{
		...
		Main.spriteBatch.Draw(star, EndPos - Main.screenPosition, null, new Color(255, 0, 60, 0), MathHelper.PiOver2, star.Size() / 2f, width / 36f, SpriteEffects.None, 0);
		Main.spriteBatch.Draw(star, EndPos - Main.screenPosition, null, new Color(255, 0, 60, 0), 0, star.Size() / 2f, width / 36f, SpriteEffects.None, 0);
	}
}
