	...
	class GreenFlame0Split : ModProjectile
	{
		...
		public override void AI()
		{
			...
			for (int j = 0; j < Main.player.Length; j++)
			{
				...
				{
					if (!Main.player[j].HasBuff(ModContent.BuffType<GreenImmune>()))
					{
			...
			if (Projectile.timeLeft == 120)
			{
				...
				for (int h = 0; h < 7; h++)
				{
					...
					Projectile.NewProjectile(null, Projectile.Center, vm + Projectile.velocity, ModContent.ProjectileType<GreenFlame0Little>(), Projectile.damage, 0, Main.myPlayer, 0, 0);
				}
				Projectile.Kill();
			}
			kb *= 0.97f;
