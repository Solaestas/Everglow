...
class BrownFlame0 : ModProjectile
{
	...
	public override void AI()
	{
		...
		for (int j = 0; j < Main.player.Length; j++)
		{
			...
			{
				if (!Main.player[j].HasBuff(ModContent.BuffType<Buffs.BrownImmune>()))
				{
	...
	Color color0 = new Color(107, 53, 0);
	Color Aimcolor = new Color(107, 53, 0);
	...
	public override void PostDraw(Color lightColor)
	{
		...
		for (int i = 1; i < Projectile.oldPos.Length; ++i)
		{
			...
			bars.Add(new Vertex2D(Projectile.oldPos[i] + normalDir * width + new Vector2(17, 17) - Main.screenPosition, new Color(107, 53, 0, 0), new Vector3(factor, 1, w)));
			bars.Add(new Vertex2D(Projectile.oldPos[i] + normalDir * -width + new Vector2(17, 17) - Main.screenPosition, new Color(107, 53, 0, 0), new Vector3(factor, 0, w)));
		}
		var Vx = new List<Vertex2D>();
		if (bars.Count > 2)
		{
			Vx.Add(bars[0]);
			var vertex = new Vertex2D((bars[0].position + bars[1].position) * 0.5f + Vector2.Normalize(Projectile.velocity) * 30, new Color(107, 53, 0, 0), new Vector3(0, 0.5f, 1));
			Vx.Add(bars[1]);
			Vx.Add(vertex);
