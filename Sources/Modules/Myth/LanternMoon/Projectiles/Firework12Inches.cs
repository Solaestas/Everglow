...
public class Firework12Inches : ModProjectile
{
				...
				for (int theta = -starsCount; theta <= starsCount; theta += 10)
				{
					if(MathF.Abs(theta) < starsCount * 0.05f)
					{
						...
						for (int phi = 0; phi <= length * starsCount; phi += 5)
						{
							...
							velocity = RodriguesRotate(velocity, axis, rot);
							FlameTrail flame = new FlameTrail(-velocity, new List<Vector3>(), velocity, color1, Main.rand.NextFloat(0.85f, 1.15f) * 0.7f, true);
							flame.FlameTimer = Main.rand.Next(-20, 0);
							Stars.Add(flame);
						}
					}
			...
			case 7:
				...
				for (int theta = -starsCount; theta <= starsCount; theta += 10)
				{
					...
					for (int phi = 0; phi <= length * starsCount; phi += 5)
					{
						Vector3 velocity = new Vector3(MathF.Cos(phi / length / starsCount * MathHelper.TwoPi) * length, MathF.Sin(theta / (float)starsCount * MathF.PI), MathF.Sin(phi / length / starsCount * MathHelper.TwoPi) * length) * 27f * Main.rand.NextFloat(0.95f, 1.05f);
						velocity = RodriguesRotate(velocity, axis, rot);
						Stars.Add(new FlameTrail(-velocity, new List<Vector3>(), velocity, color1, Main.rand.NextFloat(0.85f, 1.15f) * 1.2f, true));
						for(int x =0;x < 6;x++)
						{
	...
	{		
		...
		if (oldFlametrail.FlameTimer > 110 + oldFlametrail.ai[0])
		{
		...
		float scale;
		Vector2 v2Pos = Projection2D(drawPos3D, new Vector2(Main.screenWidth, Main.screenHeight) / 2, 2000, out scale);
		...
		foreach (FlameTrail flameTrail in Stars)
		{
			if(flameTrail.Active)
			{
				...
				for (int i = 1; i < len; i++)
				{
					...
					Vector2 old0 = Projection2D(oldPos3D0, new Vector2(Main.screenWidth, Main.screenHeight) / 2, 2000, out scale0);
					...
					Vector2 old1 = Projection2D(oldPos3D1, new Vector2(Main.screenWidth, Main.screenHeight) / 2, 2000, out scale1);
