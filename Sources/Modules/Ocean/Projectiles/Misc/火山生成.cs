...
namespace Everglow.Ocean.Projectiles.Ocean
{
    ...
    public class 火山生成 : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            int num = 0;
            if (!Main.tile[(int)Projectile.position.X / 16 - 1, (int)Projectile.position.Y / 16 - 1].HasTile && Main.tile[(int)Projectile.position.X / 16, (int)Projectile.position.Y / 16].WallType != ModContent.WallType<Everglow.Ocean.Walls.熔岩石墙>())
            {
                ...
                if (num < 3440 && num >= 0 && Main.tile[(int)Projectile.position.X / 16, (int)Projectile.position.Y / 16 + 1].TileType == ModContent.TileType<Everglow.Ocean.Tiles.玄武岩礁石>())
                {
                    WorldGen.PlaceTile((int)Projectile.position.X / 16 - 1, (int)Projectile.position.Y / 16 - 1, (ushort)ModContent.TileType<Everglow.Ocean.Tiles.喷火口>(), true, false, -1, 0);
                    int i = (int)Projectile.position.X / 16 - 1, j = (int)Projectile.position.Y / 16 - 1;
                    short num8 = (short)(Main.rand.Next(0, 8));
                    if (Main.tile[i, j].TileType == (ushort)ModContent.TileType<Everglow.Ocean.Tiles.喷火口>())
                    {
                ...
                {
                    WorldGen.PlaceTile((int)Projectile.position.X / 16 - 4, (int)Projectile.position.Y / 16 - 4, (ushort)ModContent.TileType<Everglow.Ocean.Tiles.硫磺石>(), true, false, -1, 0);
                    int i = (int)Projectile.position.X / 16 - 4, j = (int)Projectile.position.Y / 16 - 4;
                    short num8 = (short)(Main.rand.Next(0, 6));
                    if (Main.tile[i, j].TileType == (ushort)ModContent.TileType<Everglow.Ocean.Tiles.硫磺石>())
                    {
                ...
                {
                    WorldGen.PlaceTile((int)Projectile.position.X / 16 - 4, (int)Projectile.position.Y / 16 - 2, (ushort)ModContent.TileType<Everglow.Ocean.Tiles.橄榄石晶体>(), true, false, -1, 0);
                    int i = (int)Projectile.position.X / 16, j = (int)Projectile.position.Y / 16 - 2;
                    short num8 = (short)(Main.rand.Next(0, 6));
                    if (Main.tile[i, j].TileType == (ushort)ModContent.TileType<Everglow.Ocean.Tiles.橄榄石晶体>())
                    {
                        Main.tile[i, j].TileFrameX = (short)(num8 * 80);
                        Main.tile[i, j + 2].TileFrameX = (short)(num8 * 80);
