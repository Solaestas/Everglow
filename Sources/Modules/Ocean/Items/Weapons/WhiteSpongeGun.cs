    ...
    public class WhiteSpongeGun : ModItem
	{
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
            ...
            for (int h = 0; h < 3; h++)
            {
                ...
                int num = Projectile.NewProjectile(position.X + speedX + v.X, position.Y + speedY + v.Y + 5f, v.X, v.Y, ModContent.ProjectileType<Everglow.Ocean.Projectiles.WhiteSponge>(), damage * 2, knockBack, player.whoAmI, 0f, 0f);
            }
            return false;
		}
