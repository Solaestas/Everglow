...
namespace Everglow.Ocean.Items.Weapons.OceanWeapons
{
    public class OlivineAxe : ModItem
	{
		...
		{
			Dust.NewDust(new Vector2((float)hitbox.X, (float)hitbox.Y), hitbox.Width, hitbox.Height / 4, ModContent.DustType<Everglow.Ocean.Dusts.Olivine>(), 0f, 0f, 0, default(Color), 1f);
			Dust.NewDust(new Vector2((float)hitbox.X, (float)hitbox.Y), hitbox.Width, hitbox.Height / 2, ModContent.DustType<Everglow.Ocean.Dusts.Olivine>(), 0f, 0f, 0, default(Color), 1.8f);
