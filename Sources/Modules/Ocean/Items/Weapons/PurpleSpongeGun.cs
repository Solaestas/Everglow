    ...
    public class PurpleSpongeGun : ModItem
	{
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
            ...
            for (int h = 0; h < 3; h++)
            {
                ...
                int num = Projectile.NewProjectile(null, position.X + velocity.X + v.X, position.Y + velocity.Y + v.Y + 5f, v.X, v.Y, ModContent.ProjectileType<Everglow.Ocean.Projectiles.PurpleSponge>(), damage * 2, knockback, player.whoAmI, 0f, 0f);
            }
            return false;
		}
