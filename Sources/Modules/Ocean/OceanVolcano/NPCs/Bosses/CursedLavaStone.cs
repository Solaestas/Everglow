...
namespace MythMod.NPCs.VolCano
{
    ...
    public class CursedLavaStone : ModNPC
	{
        ...
        public override void AI()
        {
            if(Xa == 0)
            {
                Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0, 0, mod.ProjectileType("MagicHalo"), 0, 0, Main.myPlayer, 10, 0f);
            }
            Xa += 1;
		...
		public override void HitEffect(int hitDirection, double damage)
        {
            ...
            if(NPC.CountNPCS(mod.NPCType("LavaStone2")) <= 1)
            {
                if (!MythWorld.downedVol)
                {
                    for (int i = (int)(Main.maxTilesX * 0.62f + 445); i < (int)(Main.maxTilesX * 0.62f + 455); i++)
                    {
                        ...
                        {
                            if (Main.tile[i, j].type == mod.TileType("熔岩石"))
                            {
